 ...
 */
export function buildMessageEnumsWithCode(mapping: Record<string, string>): string {
    return Object.entries(mapping)
        .map(([key, value]) => `${value} (${key})`)
        .join(' o ');
}

export function createEnumMap<T>(enumObj: T, generateCodes: boolean = true): Record<string, T[keyof T]> {
...
export function getKeyByValueEnum<T>(value: string, enumObj: T): string | undefined {
    const entry = Object.entries(enumObj).find(([, enumValue]) => enumValue === value);
