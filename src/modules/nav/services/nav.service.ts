...
export class NavService {
  ...
  async push(codeGen: string): Promise<Shipments | null | any> {
    ...
    const shipment: Shipments =
      await this.shipmentsService.findAllRecordByCodeGen(codeGen);
  ...
  async updateMagneticCardInNavAndLeveransPretrassacionts(shipment: Shipments) {
    ...
    console.log(
      'Este fue la response al actualizar la tarjeta magnetica: ',
      response,
    );
  }
  ...
  async get(
    ...
    const shipment =
      await this.shipmentsService.getShipmentByCodeNullable(codeGen);
  ...
  async handleStatusChange(
    ...
    if (shipment === null) {
      ...
      throw new NotFoundException(
        'La transacción no existe en la API principal',
      );
    }
    ...
    const statusConfig: Record<
      string,
      { allowed: number[]; forbidden: number[] }
    > = {
    ...
    if (!statusConfig.hasOwnProperty(newStatusNav)) {
      ...
      throw new BadRequestException(
        'Se está tratando de cambiar a un status desconocido',
      );
    }
    ...
    const existingStatuses =
      await this.statusService.getStatusesForShipment(codeGen);
    const statusesToInsert = allowed.filter(
      (status) => !existingStatuses.includes(status),
    );
    if (statusesToInsert.length > 0) {
      await this.statusService.updateStatusesForShipment(
        codeGen,
        statusesToInsert,
      );
    }
  ...
  private async removeForbiddenStatuses(
    codeGen: string,
    statusesToRemove: number[],
  ) {
    for (const status of statusesToRemove) {
      if (await this.statusService.statusExistsForShipment(codeGen, status)) {
        await this.statusService.removeStatusFromShipmentByPredefined(
          codeGen,
          status,
        );
      }
  ...
  private getBaseUrl(): string {
    ...
    throw new BadRequestException(
      'Lo sentimos, el programa no esta funcionando actualmente.',
    );
  }
