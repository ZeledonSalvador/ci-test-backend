...
export class WeighbridgeConfigService {
    ...
    async createSerialComprobante(dto: CreateSerialComprobanteDto) {
        ...
        if (ultimaSerie) {
            ...
            const total = ultimaSerie.max_serialnumber - ultimaSerie.min_serialnumber + 1;

            const usados = await this.comprobanteRepo.count({
                where: {
                    noComprobante: Between(ultimaSerie.min_serialnumber, ultimaSerie.max_serialnumber),
                },
            });
        ...
        for (const serie of seriesExistentes) {
            const hayRangoSolapado =
                (dto.min_serialnumber >= serie.min_serialnumber && dto.min_serialnumber <= serie.max_serialnumber) ||
                (dto.max_serialnumber >= serie.min_serialnumber && dto.max_serialnumber <= serie.max_serialnumber) ||
                (dto.min_serialnumber <= serie.min_serialnumber && dto.max_serialnumber >= serie.max_serialnumber);
    ...
    async updateSerialComprobante(id: number, dto: UpdateSerialComprobanteDto) {
        ...
        if (dto.min_serialnumber !== undefined || dto.max_serialnumber !== undefined) {
            ...
            if (seriesSolapadas.length > 0) {
                const detalles = seriesSolapadas.map(s =>
                    `Serie ${s.id}: ${s.min_serialnumber}-${s.max_serialnumber}`,
                ).join('; ');
                throw new BadRequestException(
 ...
 */
    async listSerialesConUso(id_bascula?: number, page: number = 1, limit: number = 10) {
        const where: any = {};
        if (id_bascula) {
        ...
        for (const serie of series) {
            ...
            const anuladosCount = comprobantes.filter(
                (c) => c.status === 1,
            ).length;
            ...
            result.push({
                ...
                disponibles:
                    total !== null ? total - usadosCount : null,
                created_at: serie.created_at,
            });
        }
 ...
 */
    async listComprobantesAsignadosYAnulados(
        ...
        const parsedIdBascula = id_bascula ? parseInt(String(id_bascula), 10) : undefined;
        const parsedIdComprobseries = id_comprobseries ? parseInt(String(id_comprobseries), 10) : undefined;
        ...
        if (parsedIdComprobseries) {
            ...
            const minGlobal = Math.min(
                ...series.map((s) => s.min_serialnumber),
            );
            const maxGlobal = Math.max(
                ...series.map((s) => s.max_serialnumber),
            );
        ...
        const data = comprobantes.map((c) => {
            ...
            const productNameRaw = productCode ? getProductNameByCode(productCode) : null;
            const productName = productNameRaw ? this.formatName(productNameRaw) : null;
    ...
    async createSealSeries(dto: CreateSealSeriesDto) {
        ...
        for (const serie of seriesExistentes) {
            ...
            const hayRangoSolapado =
                (dto.min_sealnumber >= serie.min_sealnumber && dto.min_sealnumber <= serie.max_sealnumber) ||
                (dto.max_sealnumber >= serie.min_sealnumber && dto.max_sealnumber <= serie.max_sealnumber) ||
                (dto.min_sealnumber <= serie.min_sealnumber && dto.max_sealnumber >= serie.max_sealnumber);
    ...
    async updateSealSeries(id: number, dto: UpdateSealSeriesDto) {
        ...
        if (marchamosUsados.length > 0) {
            ...
            if (dto.ingenio_code !== undefined && dto.ingenio_code !== serie.ingenio_code) {
                throw new BadRequestException(
            ...
            if (dto.product_code !== undefined && dto.product_code !== serie.product_code) {
                throw new BadRequestException(
        ...
        if (dto.min_sealnumber !== undefined || dto.max_sealnumber !== undefined || dto.ingenio_code !== undefined || dto.product_code !== undefined) {
     ...
     */
    private checkRangeOverlap(min1: string, max1: string, min2: string, max2: string): boolean {
        // Caso 1: Solo números
        if (/^\d+$/.test(min1) && /^\d+$/.test(max1) && /^\d+$/.test(min2) && /^\d+$/.test(max2)) {
 ...
 */
    async listSealSeriesResumen(
        ...
        for (const serie of series) {
            ...
            const anuladosCount = marchamos.filter(
                (m) => m.status === 1,
            ).length;
            ...
            const clienteNombre = clienteNombreRaw ? this.formatName(clienteNombreRaw) : null;
            const productoNombreRaw = serie.product_code
                ? getProductNameByCode(serie.product_code)
                : null;
            const productoNombre = productoNombreRaw ? this.formatName(productoNombreRaw) : null;
    ...
    async createMarchamo(dto: CreateMarchamoDto) {
        ...
        if (marchamosExistentesActivos.length > 0) {
            const sealCodesExistentes = marchamosExistentesActivos.map(m => m.sealCode).sort();
            const sealCodesNuevos = [...sealCodes].sort();

            // Comparar si son exactamente los mismos códigos
            const sonIguales = sealCodesExistentes.length === sealCodesNuevos.length &&
                sealCodesExistentes.every((code, index) => code === sealCodesNuevos[index]);
        ...
        if (marchamosExistentes.length > 0) {
            const codigosEnUso = marchamosExistentes.map(m => m.sealCode).join(', ');
            throw new BadRequestException(
        ...
        const codigosIngenioIncompatible: Array<{ code: string; serieIngenio: string }> = [];
        const codigosProductoIncompatible: Array<{ code: string; serieProducto: string }> = [];
        ...
        for (const code of sealCodes) {
            ...
            const serie = await this.sealSeriesRepo
                ...
                .andWhere(':code BETWEEN series.min_sealnumber AND series.max_sealnumber', { code })
                .getOne();
            ...
            if (!serie) {
                ...
                codigosIngenioIncompatible.push({ code, serieIngenio: serie.ingenio_code });
            } else if (serie.product_code !== productCode) {
                // El marchamo no pertenece al producto del envío
                codigosProductoIncompatible.push({ code, serieProducto: serie.product_code });
            } else {
        ...
        if (codigosIngenioIncompatible.length > 0) {
            ...
            const ingeniosUnicos = [...new Set(codigosIngenioIncompatible.map(x => x.serieIngenio))];
            ...
            if (ingeniosUnicos.length === 1) {
                ...
                const nombreClienteReal = this.formatName(clienteReal?.name) || ingeniosUnicos[0];
        ...
        if (codigosProductoIncompatible.length > 0) {
            ...
            const productosUnicos = [...new Set(codigosProductoIncompatible.map(x => x.serieProducto))];
        ...
        for (const marchamo of saved) {
            ...
            const serie = await this.sealSeriesRepo.findOne({ where: { id: serieId } });
            if (!serie) continue;
            ...
            const stockMinimo = await this.configService.getNumber('stock_minimo_marchamos', 100);
            if (disponibles === stockMinimo) {
                const total = disponibles + new Set(marchamos.map(m => m.sealCode)).size;
                ...
                this.sendLowStockAlert('marchamo', {
                    ...
                    console.error('[createMarchamo] Error al enviar notificación de stock bajo:', err);
                });
            }
        }
    ...
    async listMarchamosPorShipment(idShipment?: number, idSealSeries?: number, page: number = 1, limit: number = 10) {
        const where: any = {};
        if (idShipment) {
     ...
     */
    private async sendCancellationNotification(type: 'comprobante' | 'seal', data: any): Promise<void> {
        try {
            const templateName = type === 'comprobante' ? 'comprobante-cancelled' : 'seal-cancelled';
            const subject = type === 'comprobante'
     ...
     */
    private async sendLowStockAlert(type: 'comprobante' | 'marchamo', data: any): Promise<void> {
        try {
    ...
    async anularMarchamoPorCodigo(dto: AnularMarchamoDto) {
        ...
        // Opción B: id_shipment proporcionado - buscar la serie automáticamente
            ...
            const serie = await this.sealSeriesRepo
                .createQueryBuilder('series')
                .where(':code BETWEEN series.min_sealnumber AND series.max_sealnumber', { code: seal_code })
    ...
    async anularMarchamosPorShipment(id_shipment: number, motivo?: string) {
        ...
        const marchamosActivos = marchamos.filter(
            (m) => m.status !== 1,
        );
 ...
 */
    ...
    private async getNextComprobanteByBascula(id_bascula: number): Promise<number> {
 ...
 */
    async assignComprobante(dto: AssignComprobanteDto) {
        ...
        if (comprobanteExistente) {
            ...
            if (Number(comprobanteExistente.noComprobante) === Number(no_comprobante)) {
                return {
                    id_shipment,
                    id_bascula,
        ...
        // Si quedan exactamente en el umbral mínimo, enviar notificación
        const stockMinimo = await this.configService.getNumber('stock_minimo_comprobantes', 100);
        if (disponibles === stockMinimo) {
            // Enviar notificación de forma asíncrona sin bloquear la respuesta
            this.sendLowStockAlert('comprobante', {
                ...
                console.error('[assignComprobante] Error al enviar notificación de stock bajo:', err);
            });
        }
 ...
 */
    ...
    async cancelComprobanteByNumero(no_comprobante: number, motivo?: string) {
        ...
        for (const serie of series) {
            if (no_comprobante >= serie.min_serialnumber && no_comprobante <= serie.max_serialnumber) {
 ...
 */
    async registrarImpresionComprobante(id_shipment: number, fecha_impresion: Date) {
        // 1. Verificar que el shipment exista
        const shipment = await this.shipmentsRepo.findOne({
            where: { id: id_shipment },
        });
