...
export class OneDriveService {
  ...
  constructor(
    ...
    this.EVIDENCE_FOLDER =
      this.configService.get<string>('EVIDENCE_FOLDER') || '';
    ...
    this.logger.log(
      `OneDrive Service inicializado con cuenta: ${this.EMAIL_FROM}`,
    );
  }
  ...
  async uploadFile(
    ...
    try {
      ...
      if (fileBuffer.length < 4 * 1024 * 1024) {
        uploadedFile = await this.simpleUpload(
          graphClient,
          uploadPath,
          fileBuffer,
          correctMimeType,
        ...
        uploadedFile = await this.largeFileUpload(
          graphClient,
          uploadPath,
          fileBuffer,
          correctMimeType,
      ...
      throw new InternalServerErrorException(
        `Error subiendo a OneDrive: ${error?.message}`,
      );
    }
  }
  ...
  async getFileMetadata(fileId: string): Promise<any> {
    try {
      ...
      this.logger.error(
        `✗ Error obteniendo metadata para ${fileId}: ${error?.message}`,
      );
      throw new InternalServerErrorException(
  ...
  async getTemporaryDownloadUrl(fileId: string): Promise<string> {
    ...
    try {
      ...
      if (hit && hit.exp > now + 30_000) {
        this.logger.debug(
          `Cache hit URL temporal para ${fileId.substring(0, 15)}...`,
        );
        return hit.url;
      }
      ...
      const graphClient = this.graphAuthService.createGraphClient();
      this.logger.debug(
        `Solicitando URL de descarga para: ${fileId.substring(0, 15)}...`,
      );
      ...
      this.logger.log(
        `✓ URL generada en ${elapsed}ms para ${fileId.substring(0, 15)}...`,
      );
      return url;
    } catch (error: any) {
      const elapsed = Date.now() - startTime;
      this.logger.error(
        `✗ Error tras ${elapsed}ms obteniendo URL para ${fileId}: ${error?.message}`,
      );
      throw new InternalServerErrorException(
        'Error generando enlace de descarga',
      );
    }
  }
  ...
  private async ensureEvidenceFolderExists(client: Client): Promise<void> {
    try {
      await client
        .api(`/users/${this.EMAIL_FROM}/drive/root:/${this.EVIDENCE_FOLDER}`)
        .get();
  ...
  private ensureCorrectMimeType(
    ...
    const ct =
      mimeMap[ext || ''] || providedMimeType || 'application/octet-stream';
    if (ct !== providedMimeType)
      this.logger.debug(
        `MIME corregido: ${providedMimeType} → ${ct} para ${fileName}`,
      );
    return ct;
  }
}
