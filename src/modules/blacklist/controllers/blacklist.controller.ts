...
export class BlacklistController {
  ...
  async createReport(
    ...
    try {
      const evidenceFiles = allFiles?.filter(
        (file) => file.fieldname === 'evidenceFiles',
      );

      return await this.blacklistService.createReport(
        createReportDto,
        evidenceFiles,
      );
    } catch (error) {
  ...
  async updateActivePenalty(
    ...
    return this.blacklistService.updateActivePenalty(
      penaltyId,
      updatePenaltyDto,
    );
  }
  ...
  async getLiberatedDrivers(
    ...
    const { data, pagination } =
      await this.blacklistService.getLiberatedDrivers(
        pageNumber,
        sizeNumber,
        license,
        liberationType,
  ...
  async getRecordById(
    ...
    const includeAttachmentsBool = includeAttachments?.toLowerCase() === 'true';
    return await this.blacklistService.getRecordById(
      id,
      includeAttachmentsBool,
    );
  }
  ...
  async getRecordsByShipment(
    ...
    const includeAttachmentsBool = includeAttachments?.toLowerCase() === 'true';
    return await this.blacklistService.getRecordsByShipment(
      shipmentId,
      includeAttachmentsBool,
    );
  }
