...
export class DashboardController {
    ...
    async getResumenEstatus(
        ...
        if (!start || !end) throw new BadRequestException('start y end son requeridos (ISO).');
        ...
        return this.dashboardService.getResumenTodosLosEstatus(startDate, endDate, productNorm, ingenioNorm);
    }
    ...
    async getPromediosAtencion(
        ...
        if (productNorm && !safe.test(productNorm)) throw new BadRequestException('product contiene caracteres inválidos.');
        if (ingenioNorm && !safe.test(ingenioNorm)) throw new BadRequestException('ingenioId contiene caracteres inválidos.');

        // Parseo de fechas (todas opcionales)
        const startDate = start ? new Date(start) : undefined;
        const endDate = end ? new Date(end) : undefined;

        if (start && isNaN(startDate!.getTime())) throw new BadRequestException('start inválido.');
        if (end && isNaN(endDate!.getTime())) throw new BadRequestException('end inválido.');
        if (startDate && endDate && startDate >= endDate) {
            throw new BadRequestException('start debe ser < end (end es exclusivo).');
        }

        return this.dashboardService.getPromediosAtencion(startDate, endDate, productNorm, ingenioNorm);
    }
    ...
    async getPromediosAtencionHoy(
        ...
        return this.dashboardService.getPromediosAtencionDelDia(productNorm, ingenioNorm);
    }
    ...
    async getPromedioDescargaHistorico(
        ...
        if (start && isNaN(s!.getTime())) throw new BadRequestException('start inválido.');
        if (end && isNaN(e!.getTime())) throw new BadRequestException('end inválido.');
        if (s && e && s >= e) throw new BadRequestException('start debe ser < end (end es exclusivo).');

        return this.dashboardService.getPromedioDescargaHistorico(
            s, e, product?.trim() || undefined, (ingenioId ?? ingenioIdAlt)?.trim() || undefined
        );
    }
    ...
    async getPromedioDescargaHoy(
        ...
        return this.dashboardService.getPromedioDescargaDelDia(productNorm, ingenioNorm);
    }
    ...
    async getPesosPorStatus(
        ...
        return this.dashboardService.getPesosPorStatusPromedioDiario(
            s, e, product?.trim() || undefined, (ingenioId ?? ingenioIdAlt)?.trim() || undefined, minStatus
        );
    }
    ...
    async getPesosPorStatusHoy(
        ...
        const toHHmm = (s?: string) => s && /^\d{1,2}$/.test(s) ? `${s.padStart(2, '0')}:00` : s;
        ...
        const base = (() => {
            if (dateStr && /^\d{4}-\d{2}-\d{2}$/.test(dateStr.trim())) {
                ...
                const dt = new Date(); dt.setFullYear(y, m - 1, d); dt.setHours(0, 0, 0, 0);
                return dt;
            }
        ...
        const parseHHmm = (s?: string) => {
            ...
            if (!m) throw new BadRequestException('hourFrom/hourTo debe ser "H" o "HH:mm".');
            const h = +m[1], mm = +(m[2] ?? 0);
            if (h < 0 || h > 23) throw new BadRequestException('Hora fuera de rango (0–23).');
            return { h, m: mm };
        };
    ...
    async getTiemposHoyDetalle(
        ...
        const toInt = (v?: string) => (v == null || v === '' ? undefined : Number(v));
        ...
        if (hour != null) {
            if (!inRange(hour)) throw new BadRequestException('hour debe estar entre 0 y 23.');
            hStart = hour;
            hEnd = hour;
        } else {
            if (hStart == null && hEnd == null) {
                ...
                if (!inRange(hStart) || !inRange(hEnd)) {
                    throw new BadRequestException('hStart/hEnd deben estar entre 0 y 23.');
                }
                if (hStart > hEnd) {
        ...
        return this.dashboardService.getTiemposDelDiaDetallado(
            productNorm, ingenioNorm, hStart!, hEnd!
        );
    }
}