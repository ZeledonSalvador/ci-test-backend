...
export class ControlSystemService {
  ...
  async getSystemControlInformationApiMiddleWare(): Promise<any> {
    try {
      ...
      return (
        'Error al conectar con la API Middleware al obtener informacion del sistema:' +
        error
      );
    }
  }
  ...
  async reStartMonitoringSystemInApiMiddlewareControlSystem() {
    try {
      ...
      console.error('Error al conectar con la API Middleware:', error);
      return (
        'Error al conectar con la API Middleware al reiniciar el sistema de monitoreo: ' +
        error
      );
    }
  }
  ...
  async startMonitoringSystemInApiMiddlewareControlSystem() {
    try {
      ...
      console.error('Error al conectar con la API Middleware:', error);
      return (
        'Error al conectar con la API Middleware al iniciar el sistema de monitoreo: ' +
        error
      );
    }
  }
  ...
  async stopMonitoringSystemInApiMiddlewareControlSystem() {
    try {
      ...
      console.error('Error al conectar con la API Middleware:', error);
      return (
        'Error al conectar con la API Middleware al detener el sistema de monitoreo: ' +
        error
      );
    }
  }
  ...
  async getDatabaseStats(): Promise<any> {
    try {
      ...
      console.error(
        '⚠️ Error al obtener estadísticas de base de datos:',
        error.message,
      );
  ...
  async getShipmentsCountByStatus(): Promise<any> {
    ...
    for (let status = 1; status <= 12; status++) {
      try {
        ...
        console.error(
          `⚠️ Error al contar envíos con status ${status}:`,
          error.message,
        );
  ...
  private validateSystemIntegrityCheck(
    ...
    const resumenControlSystem = {
      apiMainToDbConnectionStatus:
        ApiMainSystemInformation?.connectionInfo?.isConnected ?? false,
      middlewareMonitoringSystemnStatus:
        ApiMiddleWareSystemInformation?.monitoringSystem?.monitoringStatus
          ?.isServiceRunning ?? false,
      middlewareNavToDbConnectionStatus:
        ApiMiddleWareSystemInformation?.navSystem?.connectionInfo
          ?.isConnected ?? false,
      middlewareLeveransPretrassacionesToDbConnectionStatus:
        ApiMiddleWareSystemInformation?.leveransPretrassacionesSystem
          ?.connectionInfo?.isConnected ?? false,
      middlewareToApiMainConnectivity:
        ApiMiddleWareSystemInformation?.apiMainSystem?.connectivityBool ??
        false,
      middlewareToApiMainJwtValidity:
        ApiMiddleWareSystemInformation?.apiMainSystem?.jwtConnectivityInfo
          ?.valid ?? false,
      systemIntegrityCheck: false,
    };

    resumenControlSystem.systemIntegrityCheck = Object.entries(
      resumenControlSystem,
    )
    ...
    const routes = controllers.map((wrapper) => {
      ...
      return methods.map((method) => {
        ...
        const paramsMetadata =
          this.reflector.get('parameters', prototype[method]) || [];
        const returnType = Reflect.getMetadata(
          'design:returntype',
          prototype,
          method,
        );
        const httpMethods = Object.keys(RequestMethod).filter((key) =>
          isNaN(Number(key)),
        );
