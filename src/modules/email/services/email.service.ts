...
export class EmailService {
  ...
  constructor(
    ...
    this.portalClientesUrl = this.configService.get<string>('PORTAL_CLIENTES') || 'http://192.168.200.112:2053';
    this.quickpassUrl = this.configService.get<string>('QUICKPASS') || 'https://sv-svr-almapp05.almapac.com:2057/Login';
  ...
  private initializeGraphClient(): void {
    try {
      if (!this.fromEmail) {
        this.logger.error('La variable EMAIL_FROM es requerida pero no está configurada');
        return;
      }
      ...
      this.logger.error('Error al inicializar Graph Client en EmailService:', error);
    }
  }

  async sendEmail(sendEmailDto: SendEmailDto): Promise<SendEmailResult> {
    if (!this.graphClient) {
      throw new InternalServerErrorException('Servicio de email no inicializado correctamente');
    }
    ...
    try {
      ...
      this.logger.log(`Email enviado exitosamente a ${to} con asunto: ${subject}`);
      ...
      throw new InternalServerErrorException(`Error al enviar email: ${error.message}`);
    }
  }
  ...
  async sendNotification(config: {
    ...
    try {
      ...
      throw new InternalServerErrorException(`Error al enviar notificación: ${error.message}`);
    }
  }

  private validateNotificationConfig(config: any): void {
    const { templateName, templatesByRole, subject, templateData, codigos, roles, roleFilters, mixedTargets } = config;

    // Validar templates
    if (!templateName && (!templatesByRole || Object.keys(templatesByRole).length === 0)) {
      throw new BadRequestException('Se debe proporcionar templateName o templatesByRole');
    }
    ...
    const legacyModes = [codigos, roles, roleFilters].filter(mode => mode && 
      (Array.isArray(mode) ? mode.length > 0 : Object.keys(mode).length > 0)
    ).length;

    const hasMixedTargets = mixedTargets && (
      (mixedTargets.allUsersInRoles && mixedTargets.allUsersInRoles.length > 0) ||
      (mixedTargets.specificUsers && Object.keys(mixedTargets.specificUsers).length > 0) ||
  ...
  private async getRecipientsByMode(config: any): Promise<EmailUsers[]> {
    ...
    throw new BadRequestException('No se pudo determinar el modo de selección de destinatarios');
  }
  ...
  private async getRecipientsByMixedTargets(mixedTargets: {
    ...
    if (mixedTargets.allUsersInRoles && mixedTargets.allUsersInRoles.length > 0) {
      const roleRecipients = await this.emailUsersRepository
        .createQueryBuilder('user')
        .where('user.active = :active', { active: true })
        .andWhere('user.id_rol IN (:...roles)', { roles: mixedTargets.allUsersInRoles })
        .getMany();
      
      allRecipients.push(...roleRecipients);
      this.logger.log(`MixedTargets - Encontrados ${roleRecipients.length} destinatarios para todos los usuarios de roles: ${mixedTargets.allUsersInRoles.join(', ')}`);
    }

    // 2. Obtener usuarios específicos por rol y código
    if (mixedTargets.specificUsers && Object.keys(mixedTargets.specificUsers).length > 0) {
      for (const [roleStr, codigos] of Object.entries(mixedTargets.specificUsers)) {
        ...
        this.logger.log(`MixedTargets - Encontrados ${specificRecipients.length} destinatarios específicos para rol ${role} con códigos: ${codigos.join(', ')}`);
      }
    }
    ...
    if (mixedTargets.specificCodes && mixedTargets.specificCodes.length > 0) {
      const codeRecipients = await this.emailUsersRepository
        .createQueryBuilder('user')
        .where('user.active = :active', { active: true })
        .andWhere('user.codigo IN (:...codigos)', { codigos: mixedTargets.specificCodes })
        .getMany();

      allRecipients.push(...codeRecipients);
      this.logger.log(`MixedTargets - Encontrados ${codeRecipients.length} destinatarios por códigos específicos: ${mixedTargets.specificCodes.join(', ')}`);
    }

    if (allRecipients.length === 0) {
      throw new BadRequestException('No se encontraron usuarios activos para los criterios mixtos especificados');
    }
    ...
    this.logger.log(`MixedTargets - Total de destinatarios únicos: ${uniqueRecipients.length}`);
  ...
  private async getRecipientsByCodes(codigos: string[]): Promise<EmailUsers[]> {
    ...
    if (recipients.length === 0) {
      throw new BadRequestException(`No se encontraron usuarios activos con códigos: ${codigos.join(', ')}`);
    }

    this.logger.log(`Modo 1 - Encontrados ${recipients.length} destinatarios por códigos: ${codigos.join(', ')}`);
    return recipients;
  }
  ...
  private async getRecipientsByRoles(roles: number[]): Promise<EmailUsers[]> {
    ...
    if (recipients.length === 0) {
      throw new BadRequestException(`No se encontraron usuarios activos para los roles: ${roles.join(', ')}`);
    }

    this.logger.log(`Modo 2 - Encontrados ${recipients.length} destinatarios para roles: ${roles.join(', ')}`);
    return recipients;
  }

  // MODO 3: Por roles con códigos específicos
  private async getRecipientsByRoleFilters(roleFilters: Record<number, string[]>): Promise<EmailUsers[]> {
    const allRecipients: EmailUsers[] = [];
    ...
    for (const [roleStr, codigos] of Object.entries(roleFilters)) {
      ...
      this.logger.log(`Modo 3 - Encontrados ${recipients.length} destinatarios para rol ${role} con códigos: ${codigos.join(', ')}`);
    }
    ...
    if (allRecipients.length === 0) {
      const filterDesc = Object.entries(roleFilters)
        ...
        .join(' | ');
      throw new BadRequestException(`No se encontraron usuarios activos para los filtros: ${filterDesc}`);
    }
    ...
    this.logger.log(`Modo 3 - Total de destinatarios únicos: ${uniqueRecipients.length}`);
    return uniqueRecipients;
  }

  private determineTemplateForRecipient(recipient: EmailUsers, config: any): string {
  ...
  private buildRecipientTemplateData(recipient: EmailUsers, config: any): EmailTemplateData {
    return {
      ...config.templateData,
  ...
  private async sendSingleNotification(recipient: EmailUsers, config: any): Promise<NotificationDetail> {
    try {
      const templateName = this.determineTemplateForRecipient(recipient, config);
      const templateData = this.buildRecipientTemplateData(recipient, config);

      const renderedBody = await this.renderTemplate(templateName, templateData);
      ...
      this.logger.log(`Notificación enviada al rol ${recipient.id_rol} (${recipient.codigo}): ${recipient.email} usando template: ${templateName}`);
      ...
      this.logger.error(`Error al enviar notificación a ${recipient.email}: ${errorMsg}`);

      return {
        recipient: recipient.email,
        role: recipient.id_rol,
        codigo: recipient.codigo,
        templateUsed: this.determineTemplateForRecipient(recipient, config) || 'desconocido',
        status: 'failed',
        error: errorMsg
      };
    }
  ...
  private async sendNotificationsToRecipients(recipients: EmailUsers[], config: any): Promise<NotificationResult> {
    let sentCount = 0;
    const details: NotificationDetail[] = [];

    this.logger.log(`Iniciando envío de notificaciones a ${recipients.length} destinatarios`);
  ...
  private async renderTemplate(templateName: string, data: EmailTemplateData): Promise<string> {
    try {
      ...
      if (!fs.existsSync(templatePath)) {
        this.logger.error(`Template ${templateName} no encontrado en ${templatePath}`);
  ...
  private validateEmailContent(subject: string, body: string): void {
    ...
    if (subject.length > 998) {
      throw new BadRequestException('Asunto demasiado largo (máximo 998 caracteres)');
    }

    const spamWords = ['URGENTE', 'GRATIS', 'GANADOR', 'FELICITACIONES', 'HAZ CLICK AQUI', 'TIEMPO LIMITADO'];
    const upperSubject = subject.toUpperCase();
    const foundSpamWords = spamWords.filter(word => upperSubject.includes(word));
    
    if (foundSpamWords.length > 0) {
      this.logger.warn(`Palabras potencialmente spam detectadas en el asunto: ${foundSpamWords.join(', ')}`);
    }
    ...
    if (upperCasePercentage > 70) {
      this.logger.warn('Alto porcentaje de caracteres en mayúscula detectado en el asunto');
    }
  }
