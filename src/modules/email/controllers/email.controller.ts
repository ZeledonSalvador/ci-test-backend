...
import { NotificationConfig } from '../dto/email-interfaces.dto';
...
export class EmailController {
  ...
  async sendEmail(@Body() sendEmailDto: SendEmailDto) {
    ...
    if (!sendEmailDto.to || !sendEmailDto.subject || !sendEmailDto.body) {
      throw new BadRequestException(
        'Dirección de email, asunto y cuerpo son requeridos',
      );
    }
  ...
  async sendNotification(@Body() body: NotificationConfig) {
    ...
    if (
      !body.templateName &&
      (!body.templatesByRole || Object.keys(body.templatesByRole).length === 0)
    ) {
      throw new BadRequestException(
        'Se requiere templateName o templatesByRole',
      );
    }
    ...
    const legacyModes = [body.codigos, body.roles, body.roleFilters].filter(
      (mode) =>
        mode &&
    ...
    const hasMixedTargets =
      body.mixedTargets &&
      ((body.mixedTargets.allUsersInRoles &&
        body.mixedTargets.allUsersInRoles.length > 0) ||
        (body.mixedTargets.specificUsers &&
          Object.keys(body.mixedTargets.specificUsers).length > 0) ||
        (body.mixedTargets.specificCodes &&
          body.mixedTargets.specificCodes.length > 0));
    ...
    if (hasMixedTargets) {
      ...
      if (body.mixedTargets.allUsersInRoles) {
        console.log(
          `- Todos los usuarios de roles: ${body.mixedTargets.allUsersInRoles.join(', ')}`,
        );
      }
      if (body.mixedTargets.specificUsers) {
        console.log(
          `- Usuarios específicos por rol: ${JSON.stringify(body.mixedTargets.specificUsers)}`,
        );
      }
      if (body.mixedTargets.specificCodes) {
        console.log(
          `- Códigos específicos: ${body.mixedTargets.specificCodes.join(', ')}`,
        );
      }
    } else if (body.codigos) {
      console.log(
        `Usando modo 1: códigos específicos - ${body.codigos.join(', ')}`,
      );
    } else if (body.roles) {
      console.log(`Usando modo 2: roles completos - ${body.roles.join(', ')}`);
    } else if (body.roleFilters) {
      console.log(
        `Usando modo 3: filtros por rol - ${JSON.stringify(body.roleFilters)}`,
      );
    }
