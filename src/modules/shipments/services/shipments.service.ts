...
import { Repository, Between, getMetadataArgsStorage, LessThan, In, MoreThan, IsNull } from 'typeorm';
...
import { getFileTypeFromExtension, getFileTypeFromMimeType, getKeyByValueEnum } from 'src/utils/functions.util';
...
export class ShipmentsService {
    ...
    async pingMiddleware() {
        ...
        const base = process.env.MW_UPDATE_TEMP_URL
     ...
     */
    private generateUsernameForLog(userFromSession?: any, ingenioCode?: string): string {
    ...
    */
    ...
    async createShipment(
        ...
        userFromSession?: any
    ): Promise<{ shipment?: Shipments; logs: string[] } | BlacklistDetailsResponseDto> {
        const {
            codigo_gen,
            producto,
            tipo_operacion,
        ...
        console.log("¿Es igual a ProductType.MELAZA?:", producto === ProductType.MELAZA);
        ...
        const usernameForLog = this.generateUsernameForLog(userFromSession, codigo_ingenio);
        ...
        try {
            ...
            const blacklistCheck = await this.blacklistService.getBlacklistDetails(driverLicense);
            ...
            const existShipment = await this.shipmentsRepository.findOne({ where: { codeGen: codigo_gen } });
            if (existShipment) {
                const errorMsg = `El envío con código ${codigo_gen} ya existe.`;
                // NO registrar log aquí - se hará en el catch
                throw new ConflictException(errorMsg);
            } else {
                console.log("Verificación de duplicados pasada para código:", codigo_gen);
            }
            ...
            if (producto === ProductType.MELAZA && vehicle.truckType !== 'P') {
                const errorMsg = 'Para el producto MELAZA, el tipo de camión debe ser "P".';
                // NO registrar log aquí - se hará en el catch
                throw new BadRequestException(errorMsg);
            }
            if (producto !== ProductType.MELAZA && vehicle.truckType === 'P') {
                const errorMsg = 'El tipo de camión "P" solo está permitido para el producto MELAZA.';
                // NO registrar log aquí - se hará en el catch
                throw new BadRequestException(errorMsg);
            } else {
                console.log("Validación de tipo de camión pasada para producto:", producto);
            }
            ...
            if (producto !== ProductType.MELAZA && require_barrido === undefined) {
                const errorMsg = 'El campo "require_barrido" es obligatorio para productos que no son MELAZA.';
                // NO registrar log aquí - se hará en el catch
                throw new BadRequestException(errorMsg);
            } else {
                console.log("Validación de require_barrido pasada para producto:", producto);
            }
            ...
            const dupes = normalizedSeals.filter(m => (seen.has(m) ? true : (seen.add(m), false)));
            if (dupes.length > 0) {
                throw new BadRequestException(
            ...
            if (normalizedSeals.length > 0) {
                const inUse = await this.shipmentSeals
                    ...
                    .where('UPPER(seal.sealCode) IN (:...codes)', { codes: normalizedSeals })
                    .andWhere('s.currentStatus < :done', { done: 12 }) // “en proceso”
                    .getMany();

                if (inUse.length > 0) {
                    const taken = [...new Set(inUse.map(x => x.sealCode?.toString().trim().toUpperCase()))];
                    throw new BadRequestException(
            ...
            if (normalizedSeals.length > 0) {
                console.log(`Validación de marchamos pasada: ${normalizedSeals.length} marchamo(s) para producto ${producto}`);
            }
            ...
            const pesoBrutoNum = this.parseAndValidatePeso('peso_bruto', peso_bruto, { allowComma: false });
            const pesoTaraNum = this.parseAndValidatePeso('peso_tara', peso_tara, { allowComma: false });
            ...
            const shipment = this.shipmentsRepository.create({
                ...
                ingenio: client,
                mapping: mapping,
                requiresSweeping: producto === ProductType.MELAZA ? 'N' : require_barrido,
                activityNumber: "2" // El dos está quemado ahorita por que ahorita todo será azúcar y melaza
            });

            const shipmentRegisterSave = await this.shipmentsRepository.save(shipment);
    ...
    async getShipmentByCode(
        ...
        const statuses: StatusResponse[] = await this.statusService.getStatusByCodeGen(
            shipment.codeGen,
            false,
            forClients
        );
   ...
   */
    ...
    async getShipmentsByIngenio(
        ...
        if (!ingenioExists) {
            throw new NotFoundException(`Ingenio con código ${codigoIngenio} no existe`);
        }
        ...
        const [shipments, totalRecords] = await this.shipmentsRepository.findAndCount({
        ...
        for (const t of allTimes) {
            if (!bestByShipmentId.has(t.shipmentId)) bestByShipmentId.set(t.shipmentId, t);
        }

        // 4) Adjunta statuses, NAV record y downloadTime
        const withStatusesAndNav = await Promise.all(
            shipments.map(async (shipment) => {
                const statuses = await this.statusService.formatStatusByShipment(shipment, false, forClients);
        ...
        let normalized = this.normalizeShipment(withStatusesAndNav, forClients) as any[];
    ...
    async getAllShipments(
        ...
        console.log(`[ShipmentsService.getAllShipments] Parámetros:`, {
            ...
            includeAttachments, includeStatuses, forClients
        });
        ...
        const queryBuilder = this.shipmentsRepository.createQueryBuilder('shipment')
            ...
            .leftJoinAndSelect('shipment.shipmentTemperatures', 'shipmentTemperatures');

        if (includeAttachments) {
            queryBuilder.leftJoinAndSelect('shipment.shipmentAttachments', 'shipmentAttachments');
        }
        ...
        if (nameProduct && nameProduct.trim()) {
            ...
            if (productCode) {
                queryBuilder.andWhere('shipment.product = :product', { product: productCode });
            }
        }
        ...
        if (truckType && truckType.trim()) {
            ...
            if (truckCode) {
                queryBuilder.andWhere('vehicle.truckType = :truckType', { truckType: truckCode });
            }
        }
        ...
        if (excludeStatus && excludeStatus.trim()) {
            ...
            if (statusesToExclude.length > 0) {
                queryBuilder.andWhere('shipment.currentStatus NOT IN (:...excludeStatuses)', {
        ...
        try {
            ...
            const formattedShipments = await Promise.all(
                shipments.map(async (shipment): Promise<ShipmentsWithFormattedStatusesAndFormatResponseAndFormatResponse> => {
                    try {
                        ...
                        const formattedShipment: ShipmentsWithFormattedStatusesAndFormatResponseAndFormatResponse = {
                            ...
                            ...shipment,
                            statuses,
                            ...(includeAttachments && shipment.shipmentAttachments && {
                        ...
                        (formattedShipment as any).Temperature = temps.map((temp, tempIndex) => ({
                        ...
                        const basicShipment: ShipmentsWithFormattedStatusesAndFormatResponseAndFormatResponse = {
                            nameProduct: getKeyByValueEnum(shipment.product, ProductType) || 'UNKNOWN',
                            truckType: getKeyByValueEnum(shipment.vehicle?.truckType, TipoCamion) || 'UNKNOWN',
                            ...shipment,
                            statuses: [],
                            ...(includeAttachments && shipment.shipmentAttachments && {
                                shipmentAttachments: shipment.shipmentAttachments
                        ...
                        (basicShipment as any).Temperature = temps2.map((temp, tempIndex) => ({
                            registro: tempIndex + 1,
                            temperature: temp.temperature,
                            createdAt: temp.created_at
                        }));
            ...
            const normalizedShipments = this.normalizeShipment(formattedShipments, forClients);
            ...
            console.error(`[ShipmentsService.getAllShipments] Error:`, queryError.message);
    ...
    private parseStatusList(statusList: string): number[] {
        try {
            ...
            console.error(`[ShipmentsService.parseStatusList] Error parseando estados:`, error);
            return [];
        }
    }
    ...
    private async findOrCreateDriver(motorista, logs: string[]) {
        let driver = await this.driversRepository.findOne({ where: { license: motorista.licencia } });
        if (driver) {
            logs.push(`Conductor encontrado: ${driver.name} con licencia ${motorista.licencia}.`);
        } else {
            ...
            logs.push(`Conductor creado: ${driver.name} con licencia ${motorista.licencia}.`);
        }
        return driver;
    }

    private async findOrCreateVehicle(vehiculo: VehiculoDto | VehiculoUpdateDto, logs: string[]) {
    ...
    async getShipmentsByStatus(
        ...
        if (statusType === 13 && reportType) {
            queryBuilder = queryBuilder.innerJoin('shipment.dataInconsistencies', 'dataInconsistencies');
        }
        ...
        if (statusType === 13 && reportType) {
            if (reportType === 'PRECHECK') {
                queryBuilder = queryBuilder.andWhere('dataInconsistencies.inconsistencyType LIKE :precheckType', {
                    precheckType: '%"precheck":%'
                });
            } else if (reportType === 'SEALS') {
                queryBuilder = queryBuilder.andWhere('dataInconsistencies.inconsistencyType LIKE :sealsType', {
                    sealsType: '%"seals":%'
                });
            }
        }
        ...
        if (!shipments || shipments.length === 0) {
            throw new NotFoundException(`No shipments found with status type ${statusType}`);
        }

        const normalizedShipments = shipments.map(s => this.normalizeShipment(s, forClients));
        const formattedShipments = await Promise.all(
    ...
    async uploadFile(
        ...
        const shipment = await this.shipmentsRepository.findOne({ where: { codeGen: codeGen } });
    ...
    async getAttachmentsByCodeGen(
        ...
        if (!shipment) {
            throw new NotFoundException(`El envío con código ${codeGen} no fue encontrado`);
        }

        const queryBuilder = this.shipmentAttachments.createQueryBuilder('attachment')
    ...
    async findAllRecordByCodeGen(codeGen: string): Promise<Shipments | null> {
        const relations = getMetadataArgsStorage()
            .relations
            .filter(relation => relation.target === Shipments)
        ...
        if (!shipment) {
            throw new NotFoundException(`Shipment con codeGen ${codeGen} no encontrado.`);
        }
    ...
    async addMagneticCardNavToShipment(codeGen: string, magneticCard: number) {
        const shipment = await this.shipmentsRepository.findOne({ where: { codeGen } });

        if (!shipment) {
            throw new NotFoundException(`El código de generación ${codeGen} no existe`);
        }
    ...
    async setMagneticCard(codeGen: string, numberMagneticCard: number) {
        console.log(`[INFO] Intentando asignar tarjeta magnética ${numberMagneticCard} al envío ${codeGen}`);
        ...
        */
        const existingShipmentWithSameCard: Shipments = await this.shipmentsRepository.findOne({
            where: {
        ...
        if (existingShipmentWithSameCard) {
            ...
            */
            if (existingShipmentWithSameCard.codeGen === codeGen) {
                console.log(`[INFO] Tarjeta magnética ${numberMagneticCard} ya está asignada al mismo envío ${codeGen}, permitiendo reasignación`);
        ...
        */
        ...
        console.log(`[SUCCESS] Tarjeta magnética ${numberMagneticCard} asignada exitosamente al envío ${codeGen}`);
        ...
        */
        if (
            ...
            await this.navService.updateMagneticCardInNavAndLeveransPretrassacionts(shipment);
            console.log(`[INFO] Tarjeta magnética actualizada en NAV y Leverans para envío ${codeGen}`);
        }
    ...
    async setSweepingLog(codeGen: string, requiresSweeping: boolean, observation: string) {
        return this.logsShipmentsService.setSweepingLog(
            codeGen, requiresSweeping, observation
        );
    }
    ...
    async setIdNavRecord(codeGen: string, idNavRecord: number): Promise<Shipments> {
        const shipment = await this.shipmentsRepository.findOne({
            where: { codeGen },
        });
    ...
    async setIdPreTransaccionLeverans(codeGen: string, idPreTransaccionLeverans: number): Promise<Shipments> {
        const shipment = await this.shipmentsRepository.findOne({
            where: { codeGen },
        });
    ...
    public normalizeShipment<T extends Shipments | ShipmentsWithFormattedStatusesAndFormatResponseAndFormatResponse>(
        shipment: T[],
        forClients: boolean,
    ...
    public normalizeShipment<T extends Shipments | ShipmentsWithFormattedStatusesAndFormatResponseAndFormatResponse>(
        ...
        if (Array.isArray(shipment)) {
            return shipment.map(s => this.normalizeShipment(s, forClients, customConfig));
        }
    ...
    async getTruckTypeVehicleByPlateAndTrailerPlate(plate: string, trailerPlate: string) {
        const vehicle = await this.getVehicleByPlateAndTrailerPlate(plate, trailerPlate);
    ...
    async asignarBuzzer(shipmentId: number, buzzer: number): Promise<any> {
        ...
        */
        if (statusId === 2) {
            console.log('[INFO] Validando disponibilidad del buzzer para asignación...');
            ...
            */
            ...
            if (existing) {
                ...
                console.log(`[WARNING] Buzzer ${buzzer} ya está en uso activo por envío ${existing.codeGen}`);
            ...
            */
            ...
            console.log(`[SUCCESS] El buzzer ${buzzer} está disponible para asignación`);
            console.log(`[INFO] Asignando buzzer ${buzzer} al envío ${shipment.codeGen}...`);

            const result = await this.shipmentsRepository.update(shipmentId, { buzzer });
        ...
        */
        if (statusId === 4) {
            ...
            console.log(`[INFO] El buzzer ${shipment.buzzer} se mantiene como registro histórico`);
        ...
        */
        console.log(`[ERROR] Estado no permitido para operación de buzzer: ${statusId}`);
        throw new BadRequestException(
    ...
    async registrarTemperatura(codeGen: string, temperature: number) {
        ...
        if (!shipment) {
            throw new NotFoundException({ message: `No existe la transacción con codeGen ${codeGen}.` });
        }
        ...
        if (Number(temperature) < threshold) {
            ...
            const base = process.env.SERVER_MIDDLEWARE_NAME || 'http://localhost:5063/api/';
            const path = process.env.SERVER_MIDDLEWARE_UPDATE_TEMPERATURE || 'shipment/update-temperature-nav';
            ...
            try {
                ...
                nav = { message: 'No se pudo contactar la Middleware.', error: e?.message ?? String(e) };
            }
        }
    ...
    if (filter?.from) {
        qb.andWhere('st.created_at >= :from', { from: new Date(`${filter.from}T00:00:00`) });
    }
    if (filter?.to) {
        qb.andWhere('st.created_at <= :to', { to: new Date(`${filter.to}T23:59:59`) });
    }
    ...
    async getReport(filter: ShipmentTimesFilterDto) {
        ...
        throw new BadRequestException('Parámetro "mode" inválido. Solo se permite 1 o 2.');
    }


    /** ========= Genera archivo (PDF/Excel) ========= */
    async getReportFile(filter: ShipmentTimesFilterDto
        ...
        if (mode === 1) {
            ...
            if (filter.format === 'excel') {
                const buffer = await this.buildExcel({
                    title, subtitle, headers, data: mapped,
                    // Medidas específicas para este reporte
                    widths: [6, 28, 16, 28, 20, 14, 14, 12, 40],
                    align: ['center', 'left', 'left', 'left', 'left', 'center', 'center', 'center', 'left'],
                    headerRowHeight: 42,
                    rowHeight: 20,
                });
                const filename = this.buildFilename('reporte_lista_negra', 'xlsx', range);
            ...
            if (filter.format === 'pdf') {
                ...
                const filename = this.buildFilename('reporte_lista_negra', 'pdf', range);
                return { buffer, filename, mime: 'application/pdf' };
            }
            throw new BadRequestException('Formato inválido. Usa format=pdf o format=excel.');
        }
        ...
        if (filter.format === 'excel') {
            const buffer = await this.buildExcel({
                ...
                align: ['center', 'center', 'center', 'left', 'left', 'center', 'center', 'left', 'left'],
                headerRowHeight: 75,
                rowHeight: 46,
            });
            const filename = this.buildFilename('reporte_ingreso_camiones', 'xlsx', range);
            return { buffer, filename, mime: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' };
        }
        ...
        if (filter.format === 'pdf') {
            ...
            const filename = this.buildFilename('reporte_ingreso_camiones', 'pdf', range);
            return { buffer, filename, mime: 'application/pdf' };
        }
    ...
    private buildWeightsSummary(
        ...
        return weights.map((w, index) => {
            ...
            const pesoBrutoDiferencia =
                hasIngenio && pesoEntrada != null
                    ? bruto - pesoEntrada
                    : null;

            const pesoTaraDiferencia =
                hasIngenio && pesoSalida != null
                    ? tara - pesoSalida
                    : null;
    ...
    private buildFilename(
        ...
        const tag = (range?.from || range?.to) ? `_${range?.from ?? ''}_${range?.to ?? ''}` : '';
    ...
    private async buildExcel(opts: {
        ...
        if (logoPath) {
            ...
            ws.addImage(imageId, { tl: { col: 0, row: 0 }, ext: { width: 160, height: 40 } });
            hasLogo = true;
        }
        ...
        opts.data.forEach(row => {
            ...
            r.eachCell((c, colNumber) => {
                ...
                if (colAlign) {
                    c.alignment = { vertical: 'middle', horizontal: colAlign as any, wrapText: true };
                } else {
                    c.alignment = { vertical: 'middle', horizontal: (colNumber <= 3 ? 'center' : 'left'), wrapText: true };
                }
            });
        });
    ...
    private async buildPdf(opts: {
        ...
        const headerRow = headers.map(h => ({ text: h, style: 'tableHeader', alignment: 'center' }));
        ...
        // 4) Anchos de columna
        const widths = columnWidths ?? headers.map((_, i) => {
        ...
        const docDef: any = {
            ...
            footer: showPageNumbers
                ? (currentPage: number, pageCount: number) => ({
                    columns: [
                        { text: `Generado: ${new Date().toLocaleString('es-SV')}`, margin: [30, 0, 0, 0] },
            ...
            styles: {
                ...
                h1: { bold: true, fontSize: 12, alignment: 'center', margin: [0, 4, 0, 0] },
                h2: { fontSize: 9, alignment: 'center', margin: [0, 2, 0, 0] },
                tableHeader: { bold: true, fillColor: '#efefef' },
                watermark: { color: '#e0e0e0', bold: true, fontSize: 80, opacity: 0.15, alignment: 'center' }
            },
            content: [
                // Título + subtítulo
                {
                ...
                // Tabla
                {
                    table: {
                        ...
                        widths,
                        body: [
                            opts.headers.map(h => ({ text: h, bold: true, alignment: 'center' })),
                            ...opts.data.map(r =>
                                r.map((c, i) => {
                                    // si ya viene un objeto pdfmake (stack/text), lo respetamos
                                    if (c && typeof c === 'object' && (('text' in c) || ('stack' in c))) {
                                        // si no trae alignment, aplicamos el default por columna
                                        return ('alignment' in c) ? c : { ...c, alignment: i < 3 ? 'center' : 'left' };
                                    }
                                    // si viene como primitivo, lo envolvemos
                                    return { text: c ?? '', alignment: i < 3 ? 'center' : 'left' };
                                })
                            )
                        ]
                    },
  ...
  `;
    ...
    private async getShipmentTimes(filter: ShipmentTimesFilterDto) {
        ...
        if (!from && !to) {
            throw new BadRequestException('Debes enviar al menos un filtro: fecha desde o fecha hasta.');
        }
        if (from && to && new Date(from) > new Date(to)) {
            throw new BadRequestException('"from" no puede ser mayor que "to".');
        }
       ...
       END AS Tiempo`,
        ...
        if (from && to) {
            q.andWhere(new Brackets(qb =>
                qb.where('CAST(ingreso.created_at AS DATE) BETWEEN :from AND :to')
                    .orWhere('CAST(salida.created_at  AS DATE) BETWEEN :from AND :to')
            ), { from, to });
        } else if (from) {
            q.andWhere(new Brackets(qb =>
                qb.where('CAST(ingreso.created_at AS DATE) >= :from')
                    .orWhere('CAST(salida.created_at  AS DATE) >= :from')
            ), { from });
        } else if (to) {
            q.andWhere(new Brackets(qb =>
                qb.where('CAST(ingreso.created_at AS DATE) <= :to')
    ...
    async findBrixEligible(query: BrixEligibleQueryDto) {
        ...
        const buckets = new Map<string, { name: string; ingenioCode: string; total: number; data: any[] }>();
        for (const r of rows) {
            const code = r.ingenio_code ?? 'SIN_CODIGO';
            const name = r.ingenio_name ?? 'Sin nombre';
            if (!buckets.has(code)) buckets.set(code, { name, ingenioCode: code, total: 0, data: [] });
    ...
    async assignBrix(dto: AssignBrixDto) {
        ...
        if (uniqueIds.length !== 3) {
            throw new BadRequestException('Debes enviar exactamente 3 shipment_id distintos.');
        }
        ...
        if (ingenios.size !== 1) {
            throw new BadRequestException('Los 3 shipments deben pertenecer al mismo ingenio.');
        }

        // Validar estado y brix existente
        for (const s of shipments) {
            if ((s.currentStatus ?? 0) <= 8) {
                throw new BadRequestException(`El shipment ${s.id} no cumple con currentStatus > 8.`);
            }
            if (s.brix !== null && s.brix !== undefined) {
                // 🚨 Mensaje explícito
                throw new BadRequestException(`El shipment ${s.id} ya tiene un valor de brix asignado (${s.brix}).`);
            }
        }
        ...
        return await this.dataSource.transaction(async manager => {
            ...
            for (const id of uniqueIds) {
                await manager.getRepository(Shipments).createQueryBuilder()
                    .update(Shipments)
                    .set({ brix })
                    .where('id = :id', { id })
    ...
    async getDailyTransactionsStats(
        ...
        try {
            ...
            if (startDate && endDate) {
                ...
                startOfDay = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0, 0, 0);
                endOfDay = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 23, 59, 59, 999);
            }

            // Construir query con filtros opcionales
            const queryBuilder = this.shipmentsRepository.createQueryBuilder('s')
                .leftJoin('s.ingenio', 'c')
                .where('s.dateTimeCurrentStatus BETWEEN :startOfDay AND :endOfDay', { startOfDay, endOfDay })
            ...
            const totalUnits = shipments.reduce((sum, s) => sum + (Number(s.productQuantity) || 0), 0);
            ...
            const inProgress = shipments.filter(s => s.currentStatus > 2 && s.currentStatus !== 12 && s.currentStatus !== 14).length;
            const dispatched = shipments.filter(s => s.currentStatus >= 11 && s.currentStatus <= 12).length;
    ...
    async getDetailedShipmentReport(
        ...
        try {
            ...
            if (status === 1) {
                ...
                queryBuilder.andWhere('s.currentStatus > 2 AND s.currentStatus != 12 AND s.currentStatus != 14');
            } else if (status === 4) {
                // dispatched: status = 12
                queryBuilder.andWhere('s.currentStatus >= 11 AND s.currentStatus <= 12');
            } else if (status) {
                throw new BadRequestException('Estado inválido. Los estados válidos son: 1, 2, 3, 4');
            }
            ...
            const formattedData = shipments.map(shipment => {
                ...
                const entryStatus = shipmentStatuses.find(st => st.predefinedStatus?.id === 5);
                const admissionDate = entryStatus?.createdAt;

                // Buscar fecha de salida (status 11)
                const exitStatus = shipmentStatuses.find(st => st.predefinedStatus?.id === 11);
                ...
                return {
                    ...
                    preCheckDate: shipment.dateTimePrecheckeo ?
                        new Date(shipment.dateTimePrecheckeo).toISOString().split('T')[0] : null,
                    preCheckTime: shipment.dateTimePrecheckeo ?
                        new Date(shipment.dateTimePrecheckeo).toTimeString().split(' ')[0] : null,
                    createdDate: shipment.createdAt ?
                        new Date(shipment.createdAt).toISOString().split('T')[0] : null,
                    createdTime: shipment.createdAt ?
                        new Date(shipment.createdAt).toTimeString().split(' ')[0] : null,
                    ...
                    admissionDate: admissionDate ?
                        new Date(admissionDate).toISOString().split('T')[0] : null,
                    admissionTime: admissionDate ?
                        new Date(admissionDate).toTimeString().split(' ')[0] : null,
                    departureDate: departureDate ?
                        new Date(departureDate).toISOString().split('T')[0] : null,
                    departureTime: departureDate ?
                        new Date(departureDate).toTimeString().split(' ')[0] : null,
                    netWeight: Math.round((shipment.pesoBruto - shipment.pesoTara) * 1000) / 1000
                };
            });
    ...
    async getTransactionsByDateRange(
        ...
        try {
            ...
            const queryBuilder = this.shipmentsRepository.createQueryBuilder('s')
                .leftJoin('s.ingenio', 'c')
                .where('s.dateTimeCurrentStatus BETWEEN :startOfRange AND :endOfRange', { startOfRange, endOfRange })
                .select(['s.id', 's.dateTimeCurrentStatus', 's.product', 's.idNavRecord']);
            ...
            shipments.forEach(shipment => {
                if (shipment.product) {
                    productCounts[shipment.product] = (productCounts[shipment.product] || 0) + 1;
                }
            });
            ...
            shipments.forEach(shipment => {
                if (shipment.dateTimeCurrentStatus) {
                    const date = new Date(shipment.dateTimeCurrentStatus).toISOString().split('T')[0];
                    ...
                    if (shipment.product) {
                        ...
                        transactionsByDate[date].productCounts[shipment.product] =
                            (transactionsByDate[date].productCounts[shipment.product] || 0) + 1;
                    }
                }
            });
            ...
            const dailyStats = Object.entries(transactionsByDate)
                .map(([date, data]) => {
                    ...
                    if (!productId) {
                        dailyData.productBreakdown = Object.entries(data.productCounts).map(([code, count]) => ({
                            productCode: code,
            ...
            const totalDaysInRange = Math.ceil((endOfRange.getTime() - startOfRange.getTime()) / (1000 * 60 * 60 * 24)) + 1;
            const daysWithTransactions = dailyStats.length;

            // Calcular mediana de transacciones por día
            const transactionsPerDayArray = dailyStats.map(d => d.totalTransactions).sort((a, b) => a - b);
            const median = transactionsPerDayArray.length > 0
                ? transactionsPerDayArray.length % 2 === 0
                    ? (transactionsPerDayArray[transactionsPerDayArray.length / 2 - 1] + transactionsPerDayArray[transactionsPerDayArray.length / 2]) / 2
                    : transactionsPerDayArray[Math.floor(transactionsPerDayArray.length / 2)]
                : 0;

            // Encontrar día con más y menos transacciones
            const maxDay = dailyStats.length > 0
                ? dailyStats.reduce((max, day) => day.totalTransactions > max.totalTransactions ? day : max, dailyStats[0])
                : null;
            const minDay = dailyStats.length > 0
                ? dailyStats.reduce((min, day) => day.totalTransactions < min.totalTransactions ? day : min, dailyStats[0])
                : null;
            ...
            if (!productId) {
                summaryData.productTotals = Object.entries(productCounts).map(([code, count]) => ({
                    productCode: code,
                    totalTransactions: count
                }));
            } else {
    ...
    async registrarPesos(codeGen: string, navStatus: number) {
        ...
        const idNavRecordFromShipment =
            (shipment as any).idNavRecord ??
            (shipment as any).id_nav_record ??
            null;

        if (idNavRecordFromShipment == null || String(idNavRecordFromShipment).trim() === '') {
        ...
        const idNavFromResponse = navData.idNavRecord ?? Number(idNavRecordFromShipment);
