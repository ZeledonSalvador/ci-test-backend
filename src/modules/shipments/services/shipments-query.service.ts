...
export class ShipmentsQueryService {
  ...
  async getShipmentsByStatusWithFilters(
    ...
    const relations = ['driver', 'vehicle', 'ingenio', 'shipmentSeals'];
    ...
    queryBuilder = queryBuilder.where('shipment.currentStatus = :statusType', {
      statusType,
    });
    ...
    if (activity) {
      queryBuilder = queryBuilder.andWhere(
        'shipment.activityNumber = :activity',
        { activity },
      );
    }
    ...
    // Filtro por producto
    if (product) {
      queryBuilder = queryBuilder.andWhere('shipment.product = :product', {
        product,
      });
    }
    ...
    // Formatear respuesta
    const formattedShipments: ShipmentQueryResult[] = shipments.map(
      (shipment) => {
        return {
          id: shipment.id,
          ...
          ingenio: shipment.ingenio
            ...
            : null,
          shipmentSeals:
            shipment.shipmentSeals?.map((seal) => ({
              id: seal.id,
  ...
  private async getStatus5Dates(
    ...
    const statusRecords = await this.statusRepository
      ...
      .select(['shipment.id as shipmentId', 'status.createdAt as arrivedAt'])
