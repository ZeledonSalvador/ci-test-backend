...
export class ShipmentsUpdateService {
    ...
    async updateShipment(
        ...
        try {
            ...
            if (!isClientUpdate && allowedFields.length > 0) {
                isClientUpdate = true;
                console.log(`Detectada actualización de cliente por campos restringidos`);
            }
            ...
            if (!this.shipmentToUpdate) {
                ...
                await this.logError(queryRunner, 'UNKNOWN', usernameForLog, 'UNKNOWN', `Shipment ID ${idShipment} not found`);
            ...
            console.log(`Envío encontrado: ${this.shipmentToUpdate.codeGen}, Estado actual: ${this.shipmentToUpdate.currentStatus}`);
            ...
            if (!status || status.length === 0) {
                ...
                await this.logError(queryRunner, this.shipmentToUpdate.codeGen, usernameForLog, 
                    this.shipmentToUpdate.currentStatus.toString(), 'Shipment without status');
            ...
            const fieldsToUpdate = Object.keys(actualUpdateData) as (keyof Shipments)[];
            ...
            if (actualUpdateData.shipmentSeals !== undefined) {
                try {
                    await this.validateSealsUpdate(queryRunner, actualUpdateData.shipmentSeals);
                } catch (validationError) {
                    await this.logError(queryRunner, this.shipmentToUpdate.codeGen, usernameForLog, 
                        this.shipmentToUpdate.currentStatus.toString(), validationError.message);
            ...
            if (isInconsistencyResolution) {
                console.log(`DETECTADA RESOLUCIÓN DE INCONSISTENCIA para envío: ${this.shipmentToUpdate.codeGen}`);
                
                // 6.1 Buscar el estado anterior diferente a 13
                try {
                    previousStatus = await this.findPreviousNonInconsistencyStatus(queryRunner, this.shipmentToUpdate.id);
                    ...
                    await this.logError(queryRunner, this.shipmentToUpdate.codeGen, usernameForLog, 
                        this.shipmentToUpdate.currentStatus.toString(), findStatusError.message);
                ...
                resolvedInconsistency = await queryRunner.manager.findOne(DataInconsistency, {
                    where: { shipment: { id: this.shipmentToUpdate.id } },
                    relations: ['shipment']
                });

                if (resolvedInconsistency) {
                    console.log(`Reporte de inconsistencia encontrado ID: ${resolvedInconsistency.id}`);
                }
            }
            ...
            if (isInconsistencyResolution && previousStatus !== null) {
                ...
                try {
                    await this.resolveInconsistency(queryRunner, previousStatus, resolvedInconsistency);
                    console.log(`Inconsistencia resuelta. Nuevo estado: ${this.shipmentToUpdate.currentStatus}`);
                } catch (resolveError) {
                    await this.logError(queryRunner, this.shipmentToUpdate.codeGen, usernameForLog, 
                        this.shipmentToUpdate.currentStatus.toString(), 
            ...
            for (const field of fieldsToUpdate) {
                ...
                try {
                    switch (field) {
                        case 'unitMeasure':
                            await this.handleUnitMeasure(queryRunner, actualUpdateData.unitMeasure, actualUpdateData.productQuantity);
                            break;
                        case 'productQuantity':
                            await this.handleProductQuantity(actualUpdateData.productQuantity);
                            break;
                        case 'productQuantityKg':
                            await this.handleProductQuantityKg(actualUpdateData.productQuantityKg);
                            break;
                        case 'vehicle':
                            await this.handleVehicle(queryRunner, actualUpdateData.vehicle);
                            break;
                        case 'ingenio':
                            await this.handleIngenio(queryRunner, actualUpdateData.ingenio?.ingenioCode);
                            break;
                        case 'shipmentSeals':
                            // VALIDACIÓN DE POSICIÓN ESPECÍFICA
                            await this.handleUpdateSeals(queryRunner, actualUpdateData.shipmentSeals, reportedPositions);
                            break;
                        case 'driver':
                        ...
                        default:
                            ...
                            if (actualUpdateData[field] !== undefined) {
                                (this.shipmentToUpdate as any)[field] = actualUpdateData[field]!;
            ...
            if (!isInconsistencyResolution) {
                const currentShipmentState = await queryRunner.manager.findOne(Shipments, {
                    where: { id: idShipment },
                    select: ['currentStatus', 'updatedAt']
                });

                if (currentShipmentState.currentStatus !== originalStatus) {
                    const conflictMsg = 'El envío fue modificado por otro usuario. Intente nuevamente.';
                ...
                console.log(`Skipping optimistic validation for inconsistency resolution`);
            }

            // 10. ACTUALIZAR TIMESTAMP Y GUARDAR
            this.shipmentToUpdate.updatedAt = new Date();
            const savedShipment = await queryRunner.manager.save(Shipments, this.shipmentToUpdate);
            
            console.log(`Envío actualizado exitosamente: ${savedShipment.codeGen}, Nuevo estado: ${savedShipment.currentStatus}`);

            // 11. *** REGISTRAR LOG DE ÉXITO ***
            await this.transactionLogsService.createLog({
                code_gen: savedShipment.codeGen,
                json_enviado: JSON.stringify({ idShipment, updateData: actualUpdateData }),
                usuario: usernameForLog,
                estatus: savedShipment.currentStatus.toString(),
                json_modificacion: JSON.stringify({
                    action: isInconsistencyResolution ? 'INCONSISTENCY_RESOLVED' : 'SHIPMENT_UPDATED',
                    ...
                    metadata: {
                        ...
                        userSource: usuario ? 'manual' : (userFromSession ? 'session' : 'none')
                    }
                })
            });
            ...
            if (isClientUpdate) {
                await this.ingenioLogsService.logIngenioSuccess(
                    ...
                    isInconsistencyResolution ? 'INCONSISTENCY_RESOLVED' : 'UPDATE_SHIPMENT_SUCCESS',
                    { idShipment, updateData: actualUpdateData, allowedFields },
                    {
                        shipmentCodeGen: savedShipment.codeGen,
                        fieldsUpdated: fieldsToUpdate,
            ...
            const isAlreadyLogged = error.message.includes('Recurso no encontrado') || 
            ...
            if (!isAlreadyLogged) {
                await this.logError(queryRunner, this.shipmentToUpdate?.codeGen || 'UNKNOWN', usernameForLog, 
                    this.shipmentToUpdate?.currentStatus?.toString() || 'UNKNOWN', error.message);
            ...
            console.error(`Error en actualización de envío ID ${idShipment}: ${error.message}`);
            throw error;
        } finally {
     ...
     */
    private async logError(queryRunner: QueryRunner, codeGen: string, usuario: string, estatus: string, internalError: string): Promise<void> {
        try {
            await this.transactionLogsService.createLog({
                code_gen: codeGen,
     ...
     */
    private async validateSealsUpdate(queryRunner: QueryRunner, sealsToUpdate: ShipmentSeals[]): Promise<void> {
        ...
        if (sealsToUpdate.length > existingSeals.length) {
            throw new BadRequestException('Número de marchamos excede los disponibles');
        }

        // Validar que no hay códigos duplicados en los marchamos a actualizar
        const sealCodes = sealsToUpdate.map(seal => seal.sealCode?.trim().toLowerCase()).filter(code => code);
        const uniqueSealCodes = new Set(sealCodes);
        if (sealCodes.length !== uniqueSealCodes.size) {
            throw new BadRequestException('Códigos de marchamos duplicados no permitidos');
        }

        // Validar que todos los marchamos tienen códigos válidos
        const invalidSeals = sealsToUpdate.filter(seal => !seal.sealCode || seal.sealCode.trim() === '');
        if (invalidSeals.length > 0) {
            throw new BadRequestException('Códigos de marchamos inválidos');
        }

        console.log(`Validación de marchamos exitosa: ${sealsToUpdate.length} marchamos válidos para actualizar`);
    }

    /**
     * Busca el estado anterior diferente a 13 en el historial de estados
     */
    private async findPreviousNonInconsistencyStatus(queryRunner: QueryRunner, shipmentId: number): Promise<number | null> {
        try {
            ...
            if (statuses.length === 0) {
                console.warn(`⚠️ No se encontraron estados para el shipment ${shipmentId}`);
                return null;
            }
            ...
            console.warn(`⚠️ No se encontró estado anterior diferente a 13 para shipment ${shipmentId}`);
            return null;
    ...
    private async resolveInconsistency(
        ...
        try {
            ...
            const newPredefinedStatus = await queryRunner.manager.findOne(PredefinedStatuses, {
            ...
            if (inconsistencyReport) {
                ...
                console.warn(`⚠️ No se encontró reporte de inconsistencia para actualizar`);
            }
    ...
    private async handleDriver(queryRunner: QueryRunner, driverUpdate: Drivers) {
        ...
        if (driverUpdate.license && driverUpdate.license.trim() !== '' && driverUpdate.license !== currentDriver.license) {
            ...
            wasUpdated = true;
            updateFields.push(`license: "${oldLicense}" -> "${currentDriver.license}"`);
        }

        // Actualizar name si se proporciona y es diferente
        if (driverUpdate.name && driverUpdate.name.trim() !== '' && driverUpdate.name !== currentDriver.name) {
        ...
        if (wasUpdated) {
            ...
            console.log(`Conductor actualizado ID ${currentDriver.id}: ${updateFields.join(', ')}`);
        } else {
            console.log(`Conductor sin cambios ID ${currentDriver.id}: ${currentDriver.license} - ${currentDriver.name}`);
        }
     ...
     */
    private async handleVehicle(queryRunner: QueryRunner, vehicleData?: Vehicles) {
        ...
        if (vehicleData.plate && vehicleData.plate.trim() !== '' && vehicleData.plate !== currentVehicle.plate) {
        ...
        if (vehicleData.trailerPlate !== undefined && vehicleData.trailerPlate !== currentVehicle.trailerPlate) {
            const newTrailerPlate = vehicleData.trailerPlate;

            // Debe comenzar con "RE" seguido de números (si no es vacío/null)
            if (newTrailerPlate && newTrailerPlate.trim() !== '' && !/^RE\d+$/.test(newTrailerPlate.trim())) {
                throw new BadRequestException(
            ...
            wasUpdated = true;
            updateFields.push(`trailerPlate: "${oldTrailerPlate}" -> "${currentVehicle.trailerPlate}"`);
        }

        // Actualizar truckType si se proporciona y es diferente
        if (vehicleData.truckType && vehicleData.truckType.trim() !== '' && vehicleData.truckType !== currentVehicle.truckType) {
            const oldTruckType = currentVehicle.truckType;
            currentVehicle.truckType = vehicleData.truckType.trim();
            wasUpdated = true;
            updateFields.push(`truckType: "${oldTruckType}" -> "${currentVehicle.truckType}"`);
        }
        ...
        if (wasUpdated) {
            ...
            console.log(`Vehículo actualizado ID ${currentVehicle.id}: ${updateFields.join(', ')}`);
        } else {
            console.log(`Vehículo sin cambios ID ${currentVehicle.id}: ${currentVehicle.plate} - ${currentVehicle.truckType}`);
        }
    ...
    private async handleUnitMeasure(queryRunner: QueryRunner, unitMeasure?: string, productQuantity?: number) {
        ...
        const cantidadProducto = productQuantity !== undefined ? productQuantity : this.shipmentToUpdate.productQuantity;
    ...
    private async handleIngenio(queryRunner: QueryRunner, ingenioCode?: string) {
        ...
        const ingenio = await queryRunner.manager.findOne(Clients, { where: { ingenioCode } });
        if (!ingenio) {
     ...
     */
    private async handleUpdateSeals(queryRunner: QueryRunner, sealsToUpdate?: ShipmentSeals[], reportedPositions?: string[]): Promise<void> {
        if (!sealsToUpdate || sealsToUpdate.length === 0) {
        ...
        if (reportedPositions && reportedPositions.length > 0) {
            console.log(`Actualizando marchamos por posiciones específicas: ${reportedPositions.join(', ')}`);
            ...
            for (let i = 0; i < reportedPositions.length; i++) {
                ...
                const positionMatch = position.match(/marchamo(\d+)/i);
                 ...
                 if (positionMatch) {
                    ...
                    if (sealIndex >= 0 && sealIndex < existingSeals.length && sealIndex < sealsToUpdate.length) {
                        const existingSeal = existingSeals[sealIndex];
                        const newSealData = sealsToUpdate[sealIndex];

                        console.log(`   Actualizando seal por posición específica ${position}:`);
                        ...
                        updatePromises.push(queryRunner.manager.save(ShipmentSeals, existingSeal));
                    } else {
                        console.warn(`Posición ${position} fuera de rango para marchamos existentes`);
                    }
                }
            }
        } else {
            // Actualización secuencial normal (comportamiento original)
            console.log(`Actualizando marchamos por orden secuencial (comportamiento original)`);
        ...
        if (updatePromises.length > 0) {
            ...
            console.log(`${updatePromises.length} marchamos actualizados exitosamente`);
        }
