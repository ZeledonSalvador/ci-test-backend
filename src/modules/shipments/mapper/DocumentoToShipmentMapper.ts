...
export default class DocumentToShipmentMapper {
    ...
    private getTruckTypeVehicleByPlateAndTrailerPlate: (plate: string, trailerPlate: string) => Promise<any>;
    ...
    public constructor(
        json: string,
        getTruckTypeVehicleByPlateAndTrailerPlate: (plate: string, trailerPlate: string) => Promise<any>,
        ...
        this.getTruckTypeVehicleByPlateAndTrailerPlate = getTruckTypeVehicleByPlateAndTrailerPlate;
    ...
    private async getPropertyConfig(): Promise<Record<string, PropertyConfig>> {
        return {
            ...
            transportista: { // FUNCIONA
                permutations: ["Transp", "traportistaingenio", "empresa", "transportista"],
            ...
            licencia: { // FUNCIONA, TODO: Definir que hacer cuando no venga
                searchBy: FindTypesDte.APPENDICE,
                throwError: "La licencia es obligatoria!"
            },
            tipo_camion: {  //FUNCIONA
                ...
                searchBy: FindTypesDte.APPENDICE,
                throwError: "No se pudo encontrar el tipo de camion con las placas proporcionadas"
            },
            unidad_medida: { // FUNCIONA
            ...
            cantidad_producto: {
                ...
                customFunctionWhenFindInDTE: async (productQuantity) => {
                    ...
                    const findUnitMessugerInDirectykey = await
                        this.findPropAgain("unidad_medida", {
                            searchBy: FindTypesDte.DIRECT_KEY,
                            setDirectKeyFromDTE: "cuerpoDocumento[0].uniMedida"
                        });
                    ...
                    if (findUnitMessugerInDirectykey == 59) {
                        ...
                        */
                        ...
                        productQuantityReturn = await this.findPropAgain("cantidad_producto", {
                            searchBy: [
                                FindTypesDte.APPENDICE,
                                FindTypesDte.All_VALUES_OBJECT
                            ],
                            ...
                            customFunctionWhenFindInDTE: async (productQuantity) => {
                                ...
                                const number = parseFloat(productQuantity.toString().replace(/,/g, ''));
                                return isNaN(number) ? null : number;
                            }
                        });
                    }
    ...
    private parseNumeric(value: any): number | null {
        ...
        if (lastSep !== -1) {
            ...
            if (decimalSep) {
                numStr =
                    numStr.slice(0, lastSep) + '.' + numStr.slice(lastSep);
            }
        }
    ...
    buildTypeFromEnum<T extends Record<string, string>>(descripcion: string, enumType: T): T[keyof T] | null {
        if (!descripcion) {
            return null;
        }
        const normalizedDescripcion = descripcion ? descripcion.trim().toLowerCase() : null;

        // 1. Buscar en los valores del enum
        const valueKey = Object.keys(enumType).find(key =>
            enumType[key as keyof T].toLowerCase() === normalizedDescripcion
        );
        ...
        const keyMatch = Object.keys(enumType).find(key =>
            key.toLowerCase() === normalizedDescripcion
        );
        ...
        const firstLetterValueMatch = Object.keys(enumType).find(key =>
            enumType[key as keyof T].charAt(0).toLowerCase() === normalizedDescripcion.charAt(0)
        );
        ...
        const firstLetterKeyMatch = Object.keys(enumType).find(key =>
            key.charAt(0).toLowerCase() === normalizedDescripcion.charAt(0)
        );
    ...
    */
        ...
        const processObject = (inputObj: any, parentKey: string = '') => {
            for (const key in inputObj) {
                ...
                if (inputObj[key] !== null && typeof inputObj[key] === 'object') {
                    if (Array.isArray(inputObj[key])) {
                        ...
                        for (let i = 0; i < inputObj[key].length; i++) {
                            if (inputObj[key][i] === null || inputObj[key][i] === '') {
                                ...
                                result.incompleteFields[`${fullKey}[${i}]`] = typeof inputObj[key][i];
                            } else {
                        ...
                        result.allFieldsComplete = result.allFieldsComplete && arrayComplete;
                    } else {
                    ...
                    setNestedProperty(result.completedObject, fullKey, "FALTA DE COMPLETAR");
    ...
    public findInObjectByKey<T>(obj: any, searchKeys: string | string[], processor?: (value: string) => T): T[] {
        let results: T[] = [];
        const keysArray = Array.isArray(searchKeys) ? searchKeys.map(k => k.toLowerCase()) : [searchKeys.toLowerCase()];
        ...
        if (typeof obj === 'string') {
            ...
            for (const item of obj) {
                results = results.concat(this.findInObjectByKey(item, searchKeys, processor));
            }
        } else if (typeof obj === 'object' && obj !== null) {
            for (const key in obj) {
                if (obj.hasOwnProperty(key)) {
                    const value = obj[key];
                    results = results.concat(this.findInObjectByKey(value, searchKeys, processor));
                }
            }
        }
    ...
    private cleanValidateAndGetSearchBy(
        ...
        config.searchBy = Array.isArray(config.searchBy) ? config.searchBy : [config.searchBy];
        ...
        if (config?.searchBy?.includes(FindTypesDte.ALL)) {
            config.searchBy = Object.values(FindTypesDte).filter(type =>
                !invalidSearchTypes.includes(type) && type !== FindTypesDte.ALL && type !== FindTypesDte.NOT_SEARCH
            );
        }

        // Validar que no se usen tipos inválidos
        if (config?.searchBy?.some(type => invalidSearchTypes.includes(type))) {
            throw new Error(`No puedes usar ${invalidSearchTypes.join(', ')} en searchBy porque son parsers.`);
        }
    ...
    private async findValueAllValuesWithTheConfig(
        ...
        config.searchBy = this.cleanValidateAndGetSearchBy(config, invalidSearchTypes);
        ...
        if (finalValue === null && config.throwError) {
            this.errorLogsInSearchAndParsers.push(
                config.throwError
            )
        }
    ...
    private async processSearches(
        ...
        if (config?.setDirectKeyFromDTE) {
            const result = this.getValueByPath(this.obJectDte, config.setDirectKeyFromDTE);
        ...
        const appendiceResult = this.findInObjectByKey(
            this.obJectDte,
            [prop, ...(config?.permutations || [])]
        )[0];
    ...
    private async executeCustomFunction(config: PropertyConfig): Promise<any> {
        ...
        const { function: externalFunction, parameters } = config.customFunctionGetExternalDataObject;
        try {
    ...
    private async applyParsers(
        ...
        if (config.setAndTryParserObject && value !== null) {
            const parsedResult = this.buildTypeFromEnum(value, config.setAndTryParserObject);
    ...
    public async build(): Promise<CreateShipmentDto | any> {
        ...
        this.shipmentDocument.tipo_operacion = await this.findValue("tipo_operacion");
        this.shipmentDocument.tipo_carga = await this.findValue("tipo_carga");
        this.shipmentDocument.vehiculo.motorista.licencia = await this.findValue("licencia");
        this.shipmentDocument.vehiculo.motorista.nombre = await this.findValue("motorista");
        this.shipmentDocument.vehiculo.placa = await this.findValue("placa");
        this.shipmentDocument.vehiculo.placa_remolque = await this.findValue("placa_remolque");
        this.shipmentDocument.vehiculo.tipo_camion = await this.findValue("tipo_camion")
        this.shipmentDocument.transportista.nombre = await this.findValue("transportista");
        this.shipmentDocument.codigo_ingenio = await this.findValue("codigo_ingenio");
        this.shipmentDocument.cantidad_producto = await this.findValue("cantidad_producto");
        this.shipmentDocument.unidad_medida = await this.findValue("unidad_medida");
        this.shipmentDocument.require_barrido = await this.findValue("require_barrido");
        ...
        this.shipmentDocument.peso_bruto = await this.findPropAgain("peso_bruto", {
            ...
            customFunctionWhenFindInDTE: async (pesoBruto) => {
                pesoBruto = Array.isArray(pesoBruto) ?
                    pesoBruto.join('') :
                    pesoBruto;
        ...
        this.shipmentDocument.peso_tara = await this.findPropAgain("peso_tara", {
            ...
            customFunctionWhenFindInDTE: async (pesoTara) => {
                pesoTara = Array.isArray(pesoTara) ?
                    pesoTara.join('') :
                    pesoTara;
