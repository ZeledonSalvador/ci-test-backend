...
export default class CSVEncoderApendice {
  ...
  private static readonly WATERMARK =
    '_ALMAPAC_API_INTERNAL_HEADER_WATERMARK_SERIALIZER_';
  ...
  decode(): { [key: string]: string | string[] } {
    if (!CSVEncoderApendice.isDecodable(this.toJSON())) {
      throw new Error(
        'Este objeto no contiene la marca de agua y no puede ser decodificado.',
      );
    }

    this.campo = this.campo.replace(`:${CSVEncoderApendice.WATERMARK}`, '');

    return this.valor.split(', ').reduce(
      (acc, pair) => {
        const [key, value] = pair.split(':');
        const cleanedKey = key.trim();
        const cleanedValue = value.trim().replace(/"/g, '');
        acc[cleanedKey] = cleanedValue.includes(',')
          ? cleanedValue.split(',')
          : cleanedValue;
        return acc;
      },
  ...
  /* 
        ...
        funciona bien
    */
  static transformDataApendicekeysOldVersion(
    input: ApendiceDTO,
  ): TransformedData<ApendiceDTO['campo']> {
  ...
  static buildApendice(apendices: ApendiceDTO[]): {
    ...
    apendices.forEach((apendice) => {
      if (CSVEncoderApendice.isDecodable(apendice)) {
        ...
        const transformedData =
          this.transformDataApendicekeysOldVersion(apendice);
        result[apendice.campo] =
          typeof transformedData === 'object' && transformedData[apendice.campo]
