...
export class MotoristaDto {
  @IsString({
    message: 'La licencia del motorista es requerida y debe ser una cadena.',
  })
  @IsNotEmpty({ message: 'La licencia es un campo requerido.' })
  @Transform(
    ({ value }) => {
      console.log(
        'Transformando licencia sin guiones: ',
        value?.replace(/-/g, ''),
      );
  ...
  @IsString({
    message: 'El nombre del motorista es requerido y debe ser una cadena.',
  })
...
export class VehiculoDto {
  ...
  @IsString({
    message: 'La placa del vehículo es requerida y debe ser una cadena.',
  })
  @IsNotEmpty({ message: 'La placa del vehículo es un campo requerido.' })
  @Transform(({ value }) => value?.trim())
  @Matches(/^C\d+$/, {
    message:
      'La placa del cabezal debe comenzar con "C" seguido de números, sin espacios.',
  })
  placa: string;

  @IsString({
    message: 'La placa del remolque es requerida y debe ser una cadena.',
  })
  @IsNotEmpty({ message: 'La placa del remolque es un campo requerido.' })
  @Transform(({ value }) => value?.trim())
  @Matches(/^RE\d+$/, {
    message:
      'La placa del remolque debe comenzar con "RE" seguido de números, sin espacios.',
  })
  ...
  @IsEnum(TipoCamion, {
    message: `El tipo de camión debe ser uno de los siguientes: ${Object.entries(
      TipoCamion,
    )
...
export class TransportistaDto {
  @IsString({
    message: 'El nombre del transportista es requerido y debe ser una cadena.',
  })
...
export class CreateShipmentDto {
  @IsString({
    message: 'El código de generación es requerido y debe ser una cadena.',
  })
  ...
  @IsEnum(ProductType, {
    message: `El tipo de producto debe ser uno de los siguientes: ${Object.entries(
      ProductType,
    )
      .map(([key, value]) => `${value} (${key})`)
      .join(', ')}.`,
  ...
  @IsEnum(TipoOperacion, {
    message: `El tipo de operación debe ser uno de los siguientes: ${Object.entries(
      TipoOperacion,
    )
      .map(([key, value]) => `${value} (${key})`)
      .join(', ')}.`,
  ...
  @IsEnum(TipoCarga, {
    message: `El tipo de carga debe ser uno de los siguientes: ${Object.entries(
      TipoCarga,
    )
      .map(([key, value]) => `${value} (${key})`)
      .join(', ')}.`,
  ...
  @IsString({
    message: 'El código del ingenio es requerido y debe ser una cadena.',
  })
  ...
  @IsString({
    message: 'La unidad de medida es requerida y debe ser una cadena.',
  })
  ...
  @Transform(({ value }) => {
    return (
      RequiresSweepingType[value as keyof typeof RequiresSweepingType] || value
    );
  })
  @IsOptional()
  @Transform(({ value }) => {
  ...
  @IsEnum(RequiresSweepingType, {
    message: `El require_barrido debe ser uno de los siguientes: ${Object.entries(
      RequiresSweepingType,
    )
      .map(([key, value]) => `${value} (${key})`)
      .join(', ')}.`,
  })
  ...
  @IsString({
    each: true,
    message: 'Cada marchamo debe ser una cadena de texto.',
  })
  @IsNotEmpty({ each: true, message: 'Los marchamos no pueden estar vacíos.' })
  @Transform(({ value }) =>
    Array.isArray(value) ? value.map((v: string) => v?.trim()) : value,
  )
  marchamos: string[];
  ...
  @IsNumber(
    ...
    {
      message:
        'El peso bruto debe ser un dato numérico. No agregues letras ni símbolos (por ejemplo, "kg"). Ej.: 38500 o 38500.75.',
    },
  )
  ...
  @IsNumber(
    ...
    {
      message:
        'El peso tara debe ser un dato numérico. No agregues letras ni símbolos (por ejemplo, "kg"). Ej.: 38500 o 38500.75.',
    },
  )
