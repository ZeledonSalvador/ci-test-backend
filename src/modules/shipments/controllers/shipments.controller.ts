...
export class ShippingController {
  ...
  async loadMapDte(
    ...
    const mapper = new DocumentToShipmentMapper(
      ...
      this.shipmentsService.getTruckTypeVehicleByPlateAndTrailerPlate.bind(
  ...
  async getReport(
    @Res() res: Response,
    @Query() filter: ShipmentTimesFilterDto,
  ) {
    if (filter.format) {
  ...
  async getShipmentsByIngenio(
    ...
    const { data, pagination } =
      await this.shipmentsService.getShipmentsByIngenio(
        codigoIngenio,
        currentPage,
        currentSize,
        start,
  ...
  async getAllShipments(
    ...
    try {
      ...
      console.log(
        `[ShippingController.getAllShipments] Respuesta: ${data.length} registros en ${duration}ms`,
      );

      res.status(200).json(data);
    } catch (error) {
      console.error(
        `[ShippingController.getAllShipments] Error:`,
        error.message,
      );
      res.status(500).json({
  ...
  async getShipmentsByStatus(
    ...
    const { data, pagination } =
      await this.shipmentsService.getShipmentsByStatus(
        statusType,
        startDate,
        endDate,
        includeAttachmentsBoolean,
  ...
  encodeJson(@Body() input: ApendiceCoderRequest): ApendiceDTO[] {
    ...
    const createApendice = (
      label: string,
      valores: { [key: string]: string | string[] },
    ): ApendiceDTO => {
  ...
  decodeApendiceJson(@Body() input: ApendiceDTO[]): {
  ...
  async setSugarTimes(
    ...
    return await this.shipmentsService.setSugarTime(codeGen, time, observation);
  }

  @Get('/sugartime/:codeGen')
  @Roles(Role.ADMIN, Role.BOT)
  async getSugarTimes(@Param('codeGen') codeGen: string) {
  ...
  @Roles(Role.ADMIN, Role.BOT)
  async getSweepingLog(@Param('codeGen') codeGen: string) {
  ...
  async asignarBuzzer(
    ...
    console.log(
      `[INFO] Intentando asignar buzzer ${body.buzzer} al envío ${codeGen}`,
    );
  ...
  @Roles(Role.ADMIN, Role.BOT)
  async getRankingMotoristas(@Query('license') license: string) {
    if (!license) {
  ...
  async registrarTemperatura(
    ...
    const idNavRecord =
      (data as any).id_nav_record ?? (data as any).idNavRecord ?? null;
  ...
  async registrarLocation(
    ...
    const result = await this.shipmentsService.registrarLocation(
      codeGen,
      trimmed,
    );
