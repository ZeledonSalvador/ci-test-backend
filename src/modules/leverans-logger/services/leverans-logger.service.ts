...
export class LeveransLoggerService {
  ...
  async login(
    ...
    if (!user) {
      ...
      user = this.leveransUsersRepository.create({
        username: leveransUsername,
      });
  ...
  async loggerUpdateStatusByUserLeveransId(
    ...
    const user = await this.leveransUsersRepository.findOne({
      where: { id: userId },
    });
    if (!user) {
    ...
    if (!shipment) {
      throw new NotFoundException(
        'No se encontró la transacción al registrar en Logger Leverans',
      );
    }
    ...
    if (!loginHistory) {
      throw new NotFoundException(
        'No se encontró historial de inicio de sesión para el usuario',
      );
    }
  ...
  async loggerDeleteByLeveransLogger(
    ...
    if (!leveransLogger) {
      throw new BadRequestException(
        'El objeto LeveransLogger no fue proporcionado',
      );
    }
    ...
    if (!existingLogger) {
      throw new NotFoundException(
        'El registro LeveransLogger no existe en la base de datos',
      );
    }
  ...
  async loggerUpdateStatusByUsernameLeverans(
    ...
    if (!user) {
      throw new NotFoundException(
        'El Usuario Leverans no fue encontrado por username',
      );
    }
