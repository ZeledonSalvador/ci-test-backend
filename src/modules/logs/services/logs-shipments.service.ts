...
export class LogsShipmentsService {
  ...
  */
   ...
   */
    private transformLogMetadataToKeyValue(logs: ShipmentLogs[]): Record<string, string>[] {
        return logs.map(log =>
            Object.fromEntries(
                log.logMetadata.map(metadata => [metadata.metadataKey, metadata.metadataValue])
            )
        );
    }
    ...
    /**
        ...
        */
    async getStatusLogs(codeGen: string, statusId?: number): Promise<Record<string, string>[]> {
        const logs = await this.getLogsByCodeGenAndType(codeGen, TypeLogsShipments.ESTATUS);
    ...
    async getSugarTimesLogs(codeGen: string): Promise<Record<string, string>[]> {
        const logs = await this.getLogsByCodeGenAndType(codeGen, TypeLogsShipments.SUGAR_TIMES);
    ...
    async getSweepingLogs(codeGen: string): Promise<Record<string, string>[]> {
        const logs = await this.getLogsByCodeGenAndType(codeGen, TypeLogsShipments.REQUIRE_SWEEPING);
        return this.transformLogMetadataToKeyValue(logs);
    }
    ...
    async getGeneralLogs(codeGen: string): Promise<Record<string, string>[]> {
        const logs = await this.getLogsByCodeGenAndType(codeGen, TypeLogsShipments.OTRO);
        return this.transformLogMetadataToKeyValue(logs);
    }
   ...
   */
    ...
    async setSweepingLog(
        ...
        return this.addLog(codeGen, TypeLogsShipments.REQUIRE_SWEEPING, `Barrido: ${requiresSweeping}`, metadata);
    }
