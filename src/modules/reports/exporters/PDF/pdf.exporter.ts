...
export class PdfExporter {
  async buildTruckEntryPdf(params: {
    ...
    params.rows.forEach((r, idx) => {
      body.push([
        ...
        { text: formatDateTime(r.fechayhora_ingreso), style: 'td', alignment: 'center' },
        { text: formatDateTime(r.fechayhora_salida), style: 'td', alignment: 'center' },
      ...
      footer: (currentPage: number, pageCount: number) => {
        return {
          ...
          columns: [
            ...
            { text: `Página ${currentPage} de ${pageCount}`, fontSize: 8, alignment: 'right', color: '#444444' },
          ],
        };
      },
      ...
      content: [
       {
        columns: [
          { width: 140, text: '' }, // “reserva” izquierda para balancear el logo
          {
            width: '*',
            stack: [
              { text: 'REPORTE DE INGRESO DE CAMIONES', style: 'title', alignment: 'center' },
              { text: `Del ${from} - ${to}`, style: 'subtitle', alignment: 'center' },
              { text: `Total: ${total}`, style: 'subtitle2', alignment: 'center', margin: [0, 2, 0, 0] },
            ],
            alignment: 'center',
          },
          {
            ...
            stack: logo
              ...
              : [{ text: 'ALMAPAC', style: 'logoFallback', alignment: 'right' }],
          },
        ],
        margin: [0, 0, 0, 10],
      },
  ...
  params.rows.forEach((r, idx) => {
    body.push([
      ...
      { text: formatDateTime(r.fecha_prechequeo), style: 'td', alignment: 'center' },
      { text: formatDateTime(r.fecha_entrada_planta), style: 'td', alignment: 'center' },
      { text: formatDateTime(r.fecha_salida_planta), style: 'td', alignment: 'center' },
    ]);
  });
    ...
    content: [
      {
        columns: [
          { width: 140, text: '' },
          {
            width: '*',
            stack: [
              { text: 'REQUIERE BARRIDO', style: 'title', alignment: 'center' },
              { text: `CLIENTE: ${ingenioname}`, style: 'subtitle', alignment: 'center' },
              { text: `Del ${from} - ${to}`, style: 'subtitle', alignment: 'center' },
              { text: `Total: ${total}`, style: 'subtitle2', alignment: 'center', margin: [0, 2, 0, 0] },
            ],
          },
          {
            width: 140,
            alignment: 'right',
            stack: logo ? [{ image: logo, width: 120 }] : [{ text: 'ALMAPAC', style: 'logoFallback', alignment: 'right' }],
          },
        ],
        margin: [0, 0, 0, 10],
      },
  ...
  return await new Promise<Buffer>((resolve, reject) => {
    try {
      pdfMake.createPdf(docDefinition).getBuffer((buffer: Uint8Array) => resolve(Buffer.from(buffer)));
    } catch (err) {
      reject(err);
    }
  });
