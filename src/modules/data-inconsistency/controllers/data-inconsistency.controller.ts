...
export class DataInconsistencyController {
  ...
  async reportInconsistency(
    @Body() reportData: ReportInconsistencyDto,
  ...
  > {
    ...
    return await this.dataInconsistencyService.getInconsistencyByShipment(
      codeGen,
    );
  }
  ...
  async getInconsistenciesByIngenio(
    ...
    if (page < 1 || size < 1) {
      throw new BadRequestException(
        'Los parámetros page y size deben ser números positivos',
      );
    }
    ...
    if (query.startDate || query.endDate) {
      if (query.startDate && query.endDate) {
        ...
        throw new BadRequestException(
          'Si proporciona fechas, tanto startDate como endDate son requeridos',
        );
      }
    }

    console.log(
      `Obteniendo inconsistencias del ingenio: ${ingenioCode} - Página: ${page}, Tamaño: ${size}${query.startDate ? `, Fechas: ${query.startDate} - ${query.endDate}` : ''}`,
    );

    const result =
      await this.dataInconsistencyService.getInconsistenciesByIngenio(
        ingenioCode,
        page,
        size,
        query,
    ...
    res.set(new PaginationInfo(result.pagination).headers);
    res.status(200).json({
      data: result.data,
      pagination: {
        page: page,
        ...
        totalPages: Math.ceil(result.pagination.count / size),
      },
    });
  }

  @Get('ingenio-nav/:ingenioNavCode')
  @Roles(Role.ADMIN, Role.BOT)
  async getInconsistenciesByIngenioNav(
    @Param('ingenioNavCode') ingenioNavCode: string,
    @Query() query: GetInconsistenciesQueryDto,
    @Res() res: Response,
  ): Promise<void> {
    const page = query.page || 1;
    ...
    console.log(
      `Obteniendo inconsistencias del ingenio (ingenioNavCode): ${ingenioNavCode} - Página: ${page}, Tamaño: ${size}${query.startDate ? `, Fechas: ${query.startDate} - ${query.endDate}` : ''}`,
    );

    const result =
      await this.dataInconsistencyService.getInconsistenciesByIngenioNav(
        ingenioNavCode,
        page,
        size,
        query,
  ...
  async getInconsistenciesStats(
    ...
    console.log('Obteniendo estadísticas de inconsistencias');

    return await this.dataInconsistencyService.getInconsistenciesStats(query);
  }

  @Get('stats/ingenio-nav/:ingenioNavCode')
  @Roles(Role.ADMIN, Role.BOT)
  async getInconsistenciesStatsByIngenioNav(
    ...
    console.log(
      `Obteniendo estadísticas del ingenio (ingenioNavCode): ${ingenioNavCode}`,
    );

    return await this.dataInconsistencyService.getInconsistenciesStatsByIngenioNav(
      ingenioNavCode,
      query,
    );
  }
  ...
  async getAllInconsistencies(
    ...
    console.log(
      `Obteniendo inconsistencias - Página: ${page}, Tamaño: ${size}${query.startDate ? `, Fechas: ${query.startDate} - ${query.endDate}` : ''}`,
    );

    const result = await this.dataInconsistencyService.getAllInconsistencies(
      page,
      size,
      query,
    );
