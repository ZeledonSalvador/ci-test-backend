...
import { InconsistencyDataStructure, SealInfo, ShipmentPrecheckInfo, SealCodeRequest } from '../interfaces/inconsistency-data.interface';
...
export class DataInconsistencyService {
    ...
    async reportInconsistency(reportData: ReportInconsistencyDto): Promise<InconsistencyResponseDto> {
        const { codeGen, reportType, comments, userId, license, trailerPlate, truckPlate, seals } = reportData;
        ...
        try {
            ...
            if (reportType === InconsistencyType.PRECHECK) {
                ...
                if (trailerPlate !== undefined) reportedData.trailerPlate = trailerPlate;
                ...
                if (license !== undefined) shipmentData.license = shipment.driver?.license || null;
                if (trailerPlate !== undefined) shipmentData.trailerPlate = shipment.vehicle?.trailerPlate || null;
                if (truckPlate !== undefined) shipmentData.truckPlate = shipment.vehicle?.plate || null;

                console.log('DEBUG - shipmentData construido (solo campos reportados):', shipmentData);
                ...
                console.log('DEBUG - newInconsistencyData.precheck:', newInconsistencyData.precheck);
                ...
                const reportedSeals: SealCodeRequest[] = seals ? seals.map(seal => ({
            ...
            if (existingReport) {
                ...
                savedReport = await this.dataInconsistencyRepository.save(existingReport);
                ...
                savedReport = await this.dataInconsistencyRepository.save(dataInconsistency);
            }
            ...
            try {
                ...
                const inconsistencyTypeText = reportType === InconsistencyType.PRECHECK 
                    ? 'Licencia o Placas' 
                    : 'Marchamos';
                ...
                const commonTemplateData = {
                    ...
                    driverName: this.normalizeTextForEmail(shipment.driver?.name) || 'N/A',
                ...
                this.notificationService.emit({
                    ...
                    console.error('Error emitiendo notificación de inconsistencia (no crítico):', err.message);
                });

                console.log(`Notificación de inconsistencia encolada - Asociación + Ingenio ${ingenioCode}: ${codeGen}`);
            } catch (notificationError) {
                console.error('Error al preparar notificación de inconsistencia:', notificationError);
            }
            ...
            const isAlreadyLogged = error.message.includes('no encontrado') || 
            ...
            console.error(`Error en reporte de inconsistencia para ${codeGen}: ${error.message}`);
            throw error;
        }
    }

    async getInconsistencyByShipment(codeGen: string): Promise<InconsistencyResponseDto | { message: string; codeGen: string; statusCode: number }> {
    ...
    private buildCountQuery(filters?: GetInconsistenciesQueryDto): SelectQueryBuilder<DataInconsistency> {
        const queryBuilder = this.dataInconsistencyRepository.createQueryBuilder('di')
    ...
    private buildDataQuery(filters?: GetInconsistenciesQueryDto): SelectQueryBuilder<DataInconsistency> {
        const queryBuilder = this.dataInconsistencyRepository.createQueryBuilder('di')
    ...
    private applyFilters(queryBuilder: SelectQueryBuilder<DataInconsistency>, filters?: GetInconsistenciesQueryDto): SelectQueryBuilder<DataInconsistency> {
        ...
        if (filters?.reportType) {
            const typeCondition = filters.reportType === 'PRECHECK' 
    ...
    async getInconsistenciesByIngenio(
        ...
        console.log('SERVICE - getInconsistenciesByIngenio - Parámetros:', { ingenioCode, page, size, filters });
        ...
        // Convertir a DTO
        const mappedData = data.map(report => new InconsistencyResponseDto(report));
    ...
    async getInconsistenciesByIngenioNav(
        ...
        console.log('SERVICE - getInconsistenciesByIngenioNav - Parámetros:', { ingenioNavCode, page, size, filters });
    ...
    async getAllInconsistencies(
        ...
        console.log('SERVICE - getAllInconsistencies - Parámetros:', { page, size, filters });
    ...
    async getInconsistenciesStats(
        ...
        console.log('SERVICE - getInconsistenciesStats - Filtros recibidos:', filters);
    ...
    async getInconsistenciesStatsByIngenioNav(
        ...
        console.log('SERVICE - getInconsistenciesStatsByIngenioNav - Filtros:', { ingenioNavCode, filters });
        ...
        console.log('SERVICE - getInconsistenciesStatsByIngenioNav - Resultado:', stats);
        return stats;
    }
    ...
    */
    private async validateInconsistentData(reportType: InconsistencyType, data: ReportInconsistencyDto, shipment: Shipments): Promise<void> {
        if (reportType === InconsistencyType.PRECHECK) {
            if (!data.license && !data.trailerPlate && !data.truckPlate) {
                throw new BadRequestException('Para reportes de prechequeado debe proporcionar al menos un campo: licencia, placa remolque o placa camión');
            }
        } else if (reportType === InconsistencyType.SEALS) {
            if (!data.seals || data.seals.length === 0) {
                throw new BadRequestException('Para reportes de marchamos debe proporcionar al menos un código de seal');
            }
            
            // Validar que todos los seals tienen sealCode
            const invalidSeals = data.seals.filter(seal => !seal.sealCode || seal.sealCode.trim() === '');
            if (invalidSeals.length > 0) {
                throw new BadRequestException('Todos los seals deben tener un código válido');
            }
            ...
            if (data.seals.length > shipmentSeals.length) {
                throw new BadRequestException(`No se pueden reportar más marchamos (${data.seals.length}) de los que tiene el envío (${shipmentSeals.length})`);
            }

            // Validar que no hay códigos duplicados en los marchamos reportados
            const reportedSealCodes = data.seals.map(seal => seal.sealCode.trim().toLowerCase());
            const uniqueSealCodes = new Set(reportedSealCodes);
            if (reportedSealCodes.length !== uniqueSealCodes.size) {
                throw new BadRequestException('No se permiten códigos de marchamos duplicados en el reporte');
            }
        }
    }

    private parseInconsistencyData(inconsistencyType: string): InconsistencyDataStructure | null {
        try {
